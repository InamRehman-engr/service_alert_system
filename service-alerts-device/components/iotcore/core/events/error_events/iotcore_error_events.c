...
ESP_EVENT_DEFINE_BASE(IOTCORE_ERROR_EVENTS_BASE);

esp_event_loop_handle_t iotcore_error_event_loop;
esp_err_t start_iotcore_error_event_loop() {
  return esp_event_loop_create(
      &(esp_event_loop_args_t){
          .queue_size = CONFIG_IOTCORE_EVENT_QUEUE_SIZE,
          .task_name = "iotcore_error_event_loop",
          .task_priority = CONFIG_IOTCORE_EVENT_LOOP_TASK_PRIORITY,
          .task_stack_size = CONFIG_IOTCORE_ERROR_EVENT_HEAP_SIZE,
      },
      &iotcore_error_event_loop);
}

esp_err_t post_iotcore_error_event(int32_t event_id, const void *event_data,
                                   size_t event_data_size) {
  return esp_event_post_to(iotcore_error_event_loop, IOTCORE_ERROR_EVENTS_BASE,
...
esp_err_t register_iotcore_error_event(int32_t event_id,
  ...
  return esp_event_handler_register_with(iotcore_error_event_loop,
                                         IOTCORE_ERROR_EVENTS_BASE, event_id,
