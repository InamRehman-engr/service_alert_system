 ...
 * That's also a singly linked list used for keeping tracks of all opened file
 * descriptor
 *
 * Shortcomings/potential issues of 32-bit hash (when
 * CONFIG_LITTLEFS_USE_ONLY_HASH) listed here:
 *     * unlink - If a different file is open that generates a hash collision,
 * it will report an error that it cannot unlink an open file.
 *     * rename - If a different file is open that generates a hash collision
 * with src or dst, it will report an error that it cannot rename an open file.
...
typedef struct _vfs_littlefs_file_t {
  lfs_file_t file;
  uint32_t hash;
  struct _vfs_littlefs_file_t
      *next; /*!< Pointer to next file in Singly Linked List */
...
typedef struct {
...
#ifdef CONFIG_LITTLEFS_SDMMC_SUPPORT
  sdmmc_card_t
      *sdcard; /*!< The SD card driver handle on which littlefs is located */
#endif

  const esp_partition_t
      *partition; /*!< The partition on which littlefs is located */
  ...
  uint16_t fd_count; /*!< The count of opened file descriptor used to speed up
                        computation */
