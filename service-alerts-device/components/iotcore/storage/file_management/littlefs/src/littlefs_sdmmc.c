...
#error                                                                         \
    "SDMMC feature is only supported in ESP-IDF v5+, see: https://github.com/joltwallet/esp_littlefs/pull/170#issuecomment-1882484668"
#else
...
int littlefs_sdmmc_read(const struct lfs_config *c, lfs_block_t block,
  ...
  esp_err_t ret = sdmmc_read_sectors(efs->sdcard, buffer, block,
                                     MIN(size / efs->cfg.read_size, 1));
  if (ret != ESP_OK) {
    ESP_LOGE(ESP_LITTLEFS_TAG,
             "Failed to read addr 0x%08lx: off 0x%08lx, block 0x%08lx, size "
             "%lu, err=0x%x",
             part_off, off, block, size, ret);
    return LFS_ERR_IO;
  }
...
int littlefs_sdmmc_write(const struct lfs_config *c, lfs_block_t block,
  ...
  esp_err_t ret = sdmmc_write_sectors(efs->sdcard, buffer, block,
                                      MIN(size / efs->cfg.prog_size, 1));
  if (ret != ESP_OK) {
    ESP_LOGE(ESP_LITTLEFS_TAG,
             "Failed to write addr 0x%08lx: off 0x%08lx, block 0x%08lx, size "
             "%lu, err=0x%x",
             part_off, off, block, size, ret);
    return LFS_ERR_IO;
  }
...
int littlefs_sdmmc_erase(const struct lfs_config *c, lfs_block_t block) {
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(ESP_LITTLEFS_TAG, "Failed to erase block %lu: ret=0x%x %s", block,
