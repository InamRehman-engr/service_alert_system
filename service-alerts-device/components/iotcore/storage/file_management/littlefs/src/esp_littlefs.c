...
#define CONFIG_LITTLEFS_FD_CACHE_REALLOC_FACTOR                                \
  2                                         /* Amount to resize FD cache by */
#define CONFIG_LITTLEFS_FD_CACHE_MIN_SIZE 4 /* Minimum size of FD cache */
#define CONFIG_LITTLEFS_FD_CACHE_HYST                                          \
  4 /* When shrinking, leave this many trailing FD slots available */
...
static ssize_t vfs_littlefs_write(void *ctx, int fd, const void *data,
                                  size_t size);
static ssize_t vfs_littlefs_read(void *ctx, int fd, void *dst, size_t size);
static ssize_t vfs_littlefs_pwrite(void *ctx, int fd, const void *src,
                                   size_t size, off_t offset);
static ssize_t vfs_littlefs_pread(void *ctx, int fd, void *dst, size_t size,
                                  off_t offset);
...
static int vfs_littlefs_readdir_r(void *ctx, DIR *pdir, struct dirent *entry,
...
static esp_err_t esp_littlefs_init_efs(esp_littlefs_t **efs,
...
static esp_err_t esp_littlefs_by_partition(const esp_partition_t *part,
                                           int *index);
...
static int vfs_littlefs_utime(void *ctx, const char *path,
                              const struct utimbuf *times);
static void vfs_littlefs_update_mtime(esp_littlefs_t *efs, const char *path);
static int vfs_littlefs_update_mtime_value(esp_littlefs_t *efs,
...
static void get_total_and_used_bytes(esp_littlefs_t *efs, size_t *total_bytes,
                                     size_t *used_bytes);
...
static inline void *esp_littlefs_calloc(size_t __nmemb, size_t __size) {
  ...
  return heap_caps_calloc(__nmemb, __size,
...
#else /* CONFIG_LITTLEFS_MALLOC_STRATEGY_DISABLE,                              \
         CONFIG_LITTLEFS_MALLOC_STRATEGY_DEFAULT or not defined */
...
esp_err_t format_from_efs(esp_littlefs_t *efs) {
  ...
  if (efs->sdcard) {
    ...
    if (ret != ESP_OK) {
      ESP_LOGE(ESP_LITTLEFS_TAG, "Failed to format SD card: 0x%x %s", ret,
               esp_err_to_name(ret));
      return ret;
    }
  ...
  if (was_mounted) {
    ...
    efs->cache_size =
        CONFIG_LITTLEFS_FD_CACHE_MIN_SIZE; // Initial size of cache; will resize
                                           // ondemand
...
void get_total_and_used_bytes(esp_littlefs_t *efs, size_t *total_bytes,
                              size_t *used_bytes) {
  sem_take(efs);
  ...
  if (used_bytes)
    *used_bytes =
        MIN(total_bytes_local, efs->cfg.block_size * lfs_fs_size(efs->fs));
  sem_give(efs);
}
...
#ifdef CONFIG_LITTLEFS_SDMMC_SUPPORT
esp_err_t esp_littlefs_sdmmc_info(sdmmc_card_t *sdcard, size_t *total_bytes,
                                  size_t *used_bytes) {
  int index;
  esp_err_t err;
...
esp_err_t esp_vfs_littlefs_register(const esp_vfs_littlefs_conf_t *conf) {
  ...
  if (conf->partition_label) {
    if (esp_littlefs_by_label(conf->partition_label, &index) != ESP_OK) {
      ESP_LOGE(ESP_LITTLEFS_TAG, "Unable to find partition \"%s\"",
    ...
    if (esp_littlefs_by_partition(conf->partition, &index) != ESP_OK) {
      ESP_LOGE(ESP_LITTLEFS_TAG, "Unable to find partition \"0x%08" PRIX32 "\"",
               conf->partition->address);
      return ESP_ERR_NOT_FOUND;
    }
  }
  ...
  if (err != ESP_OK) {
    ...
    ESP_LOGE(ESP_LITTLEFS_TAG, "Failed to register Littlefs to \"%s\"",
             conf->base_path);
    return err;
  }

  ESP_LOGV(ESP_LITTLEFS_TAG, "Successfully registered LittleFS to \"%s\"",
           conf->base_path);
  return ESP_OK;
}
...
esp_err_t
esp_vfs_littlefs_unregister_partition(const esp_partition_t *partition) {
  assert(partition);
  int index;
  ...
  ESP_LOGV(ESP_LITTLEFS_TAG, "Unregistering \"0x%08" PRIX32 "\"",
           partition->address);
  esp_err_t err = esp_vfs_unregister(_efs[index]->base_path);
  if (err != ESP_OK) {
    ESP_LOGE(ESP_LITTLEFS_TAG, "Failed to unregister \"0x%08" PRIX32 "\"",
             partition->address);
    return err;
  }
  esp_littlefs_free(&_efs[index]);
...
esp_err_t esp_littlefs_format_partition(const esp_partition_t *partition) {
  ...
  ESP_LOGV(ESP_LITTLEFS_TAG, "Formatting partition at \"0x%08" PRIX32 "\"",
           partition->address);
...
static esp_err_t esp_littlefs_by_partition(const esp_partition_t *part,
  ...
  ESP_LOGV(ESP_LITTLEFS_TAG,
           "Searching for existing filesystem for partition \"0x%08" PRIX32
           "\"",
           part->address);
  ...
  for (i = 0; i < CONFIG_LITTLEFS_MAX_PARTITIONS; i++) {
    ...
    if (part->address == p->partition->address) {
      *index = i;
      ESP_LOGV(ESP_LITTLEFS_TAG,
               "Found existing filesystem \"0x%08" PRIX32 "\" at index %d",
               part->address, *index);
      return ESP_OK;
    }
  }
  ...
  ESP_LOGV(ESP_LITTLEFS_TAG,
           "Existing filesystem \"0x%08" PRIX32 "\" not found", part->address);
  return ESP_ERR_NOT_FOUND;
}
...
static esp_err_t esp_littlefs_by_label(const char *label, int *index) {
  ...
  ESP_LOGV(ESP_LITTLEFS_TAG,
           "Searching for existing filesystem for partition \"%s\"", label);
  ...
  for (i = 0; i < CONFIG_LITTLEFS_MAX_PARTITIONS; i++) {
    ...
    if (strncmp(label, p->partition->label, 17) == 0) {
      *index = i;
      ESP_LOGV(ESP_LITTLEFS_TAG, "Found existing filesystem \"%s\" at index %d",
               label, *index);
      return ESP_OK;
    }
  }
...
#ifdef CONFIG_LITTLEFS_SDMMC_SUPPORT
static esp_err_t esp_littlefs_by_sdmmc_handle(sdmmc_card_t *handle,
                                              int *index) {
  if (!handle || !index)
    return ESP_ERR_INVALID_ARG;

  ESP_LOGV(ESP_LITTLEFS_TAG,
           "Searching for existing filesystem for SD handle %p", handle);
  ...
  for (int i = 0; i < CONFIG_LITTLEFS_MAX_PARTITIONS; i++) {
    ...
    if (p->sdcard == handle) {
      *index = i;
      ESP_LOGV(ESP_LITTLEFS_TAG, "Found existing filesystem %p at index %d",
               handle, *index);
      return ESP_OK;
    }
  }
...
static int esp_littlefs_flags_conv(int m) {
  ...
  if (m == O_RDONLY) {
    ESP_LOGV(ESP_LITTLEFS_TAG, "O_RDONLY");
    lfs_flags |= LFS_O_RDONLY;
  }

  if (m & O_APPEND) {
    ESP_LOGV(ESP_LITTLEFS_TAG, "O_APPEND");
    lfs_flags |= LFS_O_APPEND;
  }
  if (m & O_WRONLY) {
    ESP_LOGV(ESP_LITTLEFS_TAG, "O_WRONLY");
    lfs_flags |= LFS_O_WRONLY;
  }
  if (m & O_RDWR) {
    ESP_LOGV(ESP_LITTLEFS_TAG, "O_RDWR");
    lfs_flags |= LFS_O_RDWR;
  }
  if (m & O_EXCL) {
    ESP_LOGV(ESP_LITTLEFS_TAG, "O_EXCL");
    lfs_flags |= LFS_O_EXCL;
  }
  if (m & O_CREAT) {
    ESP_LOGV(ESP_LITTLEFS_TAG, "O_CREAT");
    lfs_flags |= LFS_O_CREAT;
  }
  if (m & O_TRUNC) {
    ESP_LOGV(ESP_LITTLEFS_TAG, "O_TRUNC");
...
#ifdef CONFIG_LITTLEFS_SDMMC_SUPPORT
static esp_err_t esp_littlefs_init_sdcard(esp_littlefs_t **efs,
                                          sdmmc_card_t *sdcard,
                                          bool read_only) {
  /* Allocate Context */
  *efs = esp_littlefs_calloc(1, sizeof(esp_littlefs_t));
  ...
  { /* LittleFS Configuration */
    ...
    if (((uint64_t)sdcard->csd.capacity * (uint64_t)sdcard->csd.sector_size) >
        SIZE_MAX) {
      max_allowed_blk_cnt = SIZE_MAX / sdcard->csd.sector_size;
      ESP_LOGW(ESP_LITTLEFS_TAG,
               "SD card is too big (sector=%d, count=%d; total=%llu bytes), "
               "throttling to maximum possible %u blocks",
               sdcard->csd.sector_size, sdcard->csd.capacity,
               (((uint64_t)sdcard->csd.capacity *
                 (uint64_t)sdcard->csd.sector_size)),
               max_allowed_blk_cnt);
    } else {
    ...
    (*efs)->cfg.cache_size =
        MAX(CONFIG_LITTLEFS_CACHE_SIZE,
            sdcard->csd.sector_size); // Must not be smaller than SD sector size
...
#else
#error                                                                         \
    "CONFIG_LITTLEFS_MULTIVERSION enabled but no or unknown disk version selected!"
#endif
#endif
  }
...
static esp_err_t esp_littlefs_init(const esp_vfs_littlefs_conf_t *conf) {
  ...
  if (conf->partition_label) {
    ...
    partition = esp_partition_find_first(ESP_PARTITION_TYPE_DATA,
                                         ...
                                         conf->partition_label);
    if (!partition) {
      ESP_LOGE(ESP_LITTLEFS_TAG, "partition \"%s\" could not be found",
               conf->partition_label);
    ...
    ESP_LOGV(ESP_LITTLEFS_TAG, "Using SD card handle %p for LittleFS mount",
             conf->sdcard);
    err = sdmmc_get_status(conf->sdcard);
    if (err != ESP_OK) {
      ESP_LOGE(ESP_LITTLEFS_TAG, "Failed when checking SD card status: 0x%x",
               err);
      goto exit;
    }
#endif
...
#endif
  {
    ...
    if (log_page_size % flash_page_size != 0) {
      ESP_LOGE(
          ESP_LITTLEFS_TAG,
          "LITTLEFS_PAGE_SIZE is not multiple of flash chip page size (%u)",
  ...
  if (!conf->dont_mount) {
    ...
    if (conf->format_if_mount_failed && res != LFS_ERR_OK) {
      ESP_LOGW(ESP_LITTLEFS_TAG, "mount failed, %s (%i). formatting...",
    ...
    if (res != LFS_ERR_OK) {
      ESP_LOGE(ESP_LITTLEFS_TAG, "mount failed, %s (%i)",
               esp_littlefs_errno(res), res);
      err = ESP_FAIL;
      goto exit;
    }
    ...
    if (conf->grow_on_mount) {
      ...
      if (res != LFS_ERR_OK) {
        ESP_LOGE(ESP_LITTLEFS_TAG, "FS grow failed, %s (%i)",
                 esp_littlefs_errno(res), res);
        err = ESP_FAIL;
        goto exit;
      }
...
static inline int sem_take(esp_littlefs_t *efs) {
  ...
  ESP_LOGV(ESP_LITTLEFS_TAG, "------------------------ Sem Taking [%s]",
           pcTaskGetName(NULL));
#endif
  res = xSemaphoreTakeRecursive(efs->lock, portMAX_DELAY);
#if LOG_LOCAL_LEVEL >= 5
  ESP_LOGV(ESP_LITTLEFS_TAG, "--------------------->>> Sem Taken [%s]",
           pcTaskGetName(NULL));
#endif
  return res;
}
...
#if LOG_LOCAL_LEVEL >= 5
  ESP_LOGV(ESP_LITTLEFS_TAG, "---------------------<<< Sem Give [%s]",
           pcTaskGetName(NULL));
#endif
  return xSemaphoreGiveRecursive(efs->lock);
}

/* We are using a double allocation system here, which an array and a linked
   list. The array contains the pointer to the file descriptor (the index in the
   array is what's returned to the user). The linked list is used for file
   descriptors. This means that position of nodes in the list must stay
   consistent:
   - Allocation is obvious (append to the list from the head, and realloc the
   pointers array) There is still a O(N) search in the cache for a free position
   to store
   ...
   - Deallocation is more tricky. That is, for example,
     if you need to remove node 5 in a 12 nodes list, you'll have to:
       1) Mark the 5th position as freed (if it's the last position of the array
   realloc smaller) 2) Walk the list until finding the pointer to the node O(N)
   and scrub the node so the chained list stays consistent 3) Deallocate the
   node
*/
 ...
 * @param[out]    file      pointer to a file that'll be filled with a file
 * object
 * @param[in]     path_len  the length of the filepath in bytes (including
 * terminating zero byte)
 * @return integer file descriptor. Returns -1 if a FD cannot be obtained.
 * @warning This must be called with lock taken
 */
static int esp_littlefs_allocate_fd(esp_littlefs_t *efs,
  ...
  if (efs->fd_count + 1 > efs->cache_size) {
    uint16_t new_size = (uint16_t)MIN(
        UINT16_MAX, CONFIG_LITTLEFS_FD_CACHE_REALLOC_FACTOR * efs->cache_size);
    /* Resize the cache */
    vfs_littlefs_file_t **new_cache =
        realloc(efs->cache, new_size * sizeof(*efs->cache));
    if (!new_cache) {
      ESP_LOGE(ESP_LITTLEFS_TAG, "Unable to allocate file cache");
      return -1; /* If it fails here, no harm is done to the filesystem, so it's
                    safe */
    }
    /* Zero out the new portions of the cache */
    memset(&new_cache[efs->cache_size], 0,
           (new_size - efs->cache_size) * sizeof(*efs->cache));
    efs->cache = new_cache;
  ...
  *file = esp_littlefs_calloc(1, sizeof(**file));
  ...
  if (*file == NULL) {
    /* If it fails here, the file system might have a larger cache, but it's
     * harmless, no need to reverse it */
...
static int esp_littlefs_get_fd_by_name(esp_littlefs_t *efs, const char *path) {
  ...
  for (uint16_t i = 0, j = 0; i < efs->cache_size && j < efs->fd_count; i++) {
    if (efs->cache[i]) {
      ...
      if (efs->cache[i]->hash == hash // Faster than strcmp
#ifndef CONFIG_LITTLEFS_USE_ONLY_HASH
          && strcmp(path, efs->cache[i]->path) ==
                 0 // May as well check incase of hash collision. Usually
                   // short-circuited.
#endif
      ) {
...
static int vfs_littlefs_open(void *ctx, const char *path, int flags, int mode) {
  ...
  ESP_LOGV(ESP_LITTLEFS_TAG,
           "LITTLEFS_SPIFFS_COMPAT attempting to create all directories for %s",
           path);
  mkdirs(efs, path);
#endif // CONFIG_LITTLEFS_SPIFFS_COMPAT
...
#else
#error                                                                         \
    "The use of static buffers is not currently supported by this VFS wrapper"
#endif
  ...
  if (res < 0) {
    ...
    sem_give(efs);
#ifndef CONFIG_LITTLEFS_USE_ONLY_HASH
    ESP_LOGV(ESP_LITTLEFS_TAG, "Failed to open file %s. Error %s (%d)", path,
             esp_littlefs_errno(res), res);
#else
...
#ifndef CONFIG_LITTLEFS_USE_ONLY_HASH
      ESP_LOGV(ESP_LITTLEFS_TAG,
               "Failed to sync at opening file \"%s\". Error %s (%d)",
               file->path, esp_littlefs_errno(res), res);
#else
    ESP_LOGV(ESP_LITTLEFS_TAG, "Failed to sync at opening file %d. Error %d",
             fd, res);
#endif
    }
...
#ifndef CONFIG_LITTLEFS_USE_ONLY_HASH
    ESP_LOGV(ESP_LITTLEFS_TAG,
             "Failed to write FD %d; path \"%s\". Error %s (%d)", fd,
             file->path, esp_littlefs_errno(res), res);
#else
    ESP_LOGV(ESP_LITTLEFS_TAG, "Failed to write FD %d. Error %s (%d)", fd,
             esp_littlefs_errno(res), res);
#endif
    return -1;
  }
...
static ssize_t vfs_littlefs_read(void *ctx, int fd, void *dst, size_t size) {
  ...
  if (res < 0) {
    ...
    ESP_LOGV(ESP_LITTLEFS_TAG, "Failed to read file \"%s\". Error %s (%d)",
             file->path, esp_littlefs_errno(res), res);
#else
    ESP_LOGV(ESP_LITTLEFS_TAG, "Failed to read FD %d. Error %s (%d)", fd,
             esp_littlefs_errno(res), res);
#endif
    return -1;
  }
  ...
  if ((file->file.flags & O_DIRECTORY) == 0) {
    ...
    if (res < 0) {
      ...
      ESP_LOGV(ESP_LITTLEFS_TAG, "Failed to close file \"%s\". Error %s (%d)",
               file->path, esp_littlefs_errno(res), res);
#else
    ESP_LOGV(ESP_LITTLEFS_TAG, "Failed to close Fd %d. Error %s (%d)", fd,
             esp_littlefs_errno(res), res);
#endif
      return -1;
    }
...
#ifndef CONFIG_LITTLEFS_USE_ONLY_HASH
    ESP_LOGV(ESP_LITTLEFS_TAG,
             "Failed to seek file \"%s\" to offset %08x. Error %s (%d)",
             file->path, (unsigned int)offset, esp_littlefs_errno(res),
             (int)res);
#else
    ESP_LOGV(ESP_LITTLEFS_TAG,
             "Failed to seek FD %d to offset %08x. Error (%d)", fd,
             (unsigned int)offset, (int)res);
#endif
    return -1;
  }
...
#define fail_str_1 "Failed to unlink path \"%s\"."
  ...
  if (res < 0) {
    errno = lfs_errno_remap(res);
    sem_give(efs);
    ESP_LOGV(ESP_LITTLEFS_TAG, fail_str_1 " Error %s (%d)", path,
             esp_littlefs_errno(res), res);
    return -1;
  }
...
static int vfs_littlefs_rename(void *ctx, const char *src, const char *dst) {
  ...
  if (res < 0) {
    ...
    sem_give(efs);
    ESP_LOGV(ESP_LITTLEFS_TAG,
             "Failed to rename \"%s\" -> \"%s\". Error %s (%d)", src, dst,
             esp_littlefs_errno(res), res);
    return -1;
  }
...
#else
    ESP_LOGV(ESP_LITTLEFS_TAG, "Failed to opendir \"%s\". Error %d", dir->path,
             res);
#endif
    goto exit;
  }
...
static int vfs_littlefs_closedir(void *ctx, DIR *pdir) {
  ...
  if (res < 0) {
    ...
    ESP_LOGV(ESP_LITTLEFS_TAG, "Failed to closedir \"%s\". Error %s (%d)",
             dir->path, esp_littlefs_errno(res), res);
#else
    ESP_LOGV(ESP_LITTLEFS_TAG, "Failed to closedir \"%s\". Error %d", dir->path,
             res);
#endif
    return res;
  }
...
static int vfs_littlefs_readdir_r(void *ctx, DIR *pdir, struct dirent *entry,
  ...
  if (res < 0) {
    ...
    ESP_LOGV(ESP_LITTLEFS_TAG, "Failed to readdir \"%s\". Error %s (%d)",
             dir->path, esp_littlefs_errno(res), res);
#else
    ESP_LOGV(ESP_LITTLEFS_TAG, "Failed to readdir \"%s\". Error %d", dir->path,
             res);
#endif
    return -1;
  }
...
static int vfs_littlefs_mkdir(void *ctx, const char *name, mode_t mode) {
  ...
  if (res < 0) {
    errno = lfs_errno_remap(res);
    ESP_LOGV(ESP_LITTLEFS_TAG, "Failed to mkdir \"%s\". Error %s (%d)", name,
             esp_littlefs_errno(res), res);
    return -1;
  }
  return 0;
...
static ssize_t vfs_littlefs_truncate(void *ctx, const char *path, off_t size) {
  ...
  if (res < 0) {
    ...
    ESP_LOGV(ESP_LITTLEFS_TAG, "Failed to truncate file \"%s\". Error %s (%d)",
             file->path, esp_littlefs_errno(res), res);
#else
    ESP_LOGV(ESP_LITTLEFS_TAG, "Failed to truncate FD %d. Error %s (%d)", fd,
             esp_littlefs_errno(res), res);
#endif
    res = -1;
  } else {
    ESP_LOGV(ESP_LITTLEFS_TAG, "Truncated file %s to %u bytes", path,
             (unsigned int)size);
  }
...
#ifndef CONFIG_LITTLEFS_USE_ONLY_HASH
    ESP_LOGV(ESP_LITTLEFS_TAG, "Truncated file %s to %u bytes", file->path,
             (unsigned int)size);
#else
    ESP_LOGV(ESP_LITTLEFS_TAG, "Truncated FD %d to %u bytes", fd,
             (unsigned int)size);
#endif
  }
  return res;
...
static int vfs_littlefs_update_mtime_value(esp_littlefs_t *efs,
                                           const char *path, time_t t) {
  int res;
  res = lfs_setattr(efs->fs, path, LITTLEFS_ATTR_MTIME, &t, sizeof(t));
  if (res < 0) {
    errno = lfs_errno_remap(res);
...
static time_t vfs_littlefs_get_mtime(esp_littlefs_t *efs, const char *path) {
  time_t t;
  int size;
  size = lfs_getattr(efs->fs, path, LITTLEFS_ATTR_MTIME, &t, sizeof(t));
  if (size < 0) {
