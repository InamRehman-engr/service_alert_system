...
int littlefs_esp_part_read(const struct lfs_config *c, lfs_block_t block,
  ...
  if (err) {
    ESP_LOGE(ESP_LITTLEFS_TAG, "failed to read addr %08x, size %08x, err %d",
             (unsigned int)part_off, (unsigned int)size, err);
    return LFS_ERR_IO;
  }
  return 0;
}
...
int littlefs_esp_part_write(const struct lfs_config *c, lfs_block_t block,
  ...
  if (err) {
    ESP_LOGE(ESP_LITTLEFS_TAG, "failed to write addr %08x, size %08x, err %d",
             (unsigned int)part_off, (unsigned int)size, err);
    return LFS_ERR_IO;
  }
  return 0;
...
int littlefs_esp_part_erase(const struct lfs_config *c, lfs_block_t block) {
  ...
  esp_err_t err =
      esp_partition_erase_range(efs->partition, part_off, c->block_size);
  if (err) {
    ESP_LOGE(ESP_LITTLEFS_TAG, "failed to erase addr %08x, size %08x, err %d",
             (unsigned int)part_off, (unsigned int)c->block_size, err);
    return LFS_ERR_IO;
  }
  return 0;
}
