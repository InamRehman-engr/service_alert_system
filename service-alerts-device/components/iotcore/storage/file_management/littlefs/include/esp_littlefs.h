...
typedef struct {
  ...
  const esp_partition_t
      *partition; /**< partition to use if partition_label is NULL */

#ifdef CONFIG_LITTLEFS_SDMMC_SUPPORT
  sdmmc_card_t *sdcard; /**< SD card handle to use if both esp_partition handle
                           & partition label is NULL */
#endif

  uint8_t format_if_mount_failed : 1; /**< Format the file system if it fails to
                                         mount. */
  uint8_t read_only : 1;              /**< Mount the partition as read-only. */
  uint8_t dont_mount : 1;             /**< Don't attempt to mount.*/
  uint8_t grow_on_mount : 1; /**< Grow filesystem to match partition size on
                                mount.*/
 ...
 *
 * @param   conf                      Pointer to esp_vfs_littlefs_conf_t
 ...
 *
 * @param partition_label           Optional, label of the partition to get info
 * for.
