// normal values aroud 1600mV with 10k resister
// need almost 90sec to heatup the coil
// after reading go back in normal state after around 150sec
#include "MQ_7b.h"
...
esp_err_t ADC_init(void) {
  ...
  if (CONFIG_MQ_7b_ADC_UNIT == 1)
  // ADC1 config
  {
    ESP_ERROR_CHECK(adc1_config_width(ADC_WIDTH_BIT_DEFAULT));
    ESP_ERROR_CHECK(adc1_config_channel_atten(CONFIG_MQ_7b_ADC_CHANNEL_A,
                                              ADC_EXAMPLE_ATTEN));
    ESP_ERROR_CHECK(adc1_config_channel_atten(CONFIG_MQ_7b_ADC_CHANNEL_B,
                                              ADC_EXAMPLE_ATTEN));
  } else {
    // ADC2 config
    ESP_ERROR_CHECK(adc2_config_channel_atten(CONFIG_MQ_7b_ADC_CHANNEL_A,
                                              ADC_EXAMPLE_ATTEN));
    ESP_ERROR_CHECK(adc2_config_channel_atten(CONFIG_MQ_7b_ADC_CHANNEL_B,
                                              ADC_EXAMPLE_ATTEN));
  }
  return ESP_OK;
}
void get_MQ_7b(void) {
  ...
  for (int a = 0; a < sample; a++) {
    if (CONFIG_MQ_7b_ADC_UNIT == 1) {
      adc_raw[0][0] = adc1_get_raw(CONFIG_MQ_7b_ADC_CHANNEL_A);
      adc_raw[1][0] = adc1_get_raw(CONFIG_MQ_7b_ADC_CHANNEL_B);
    } else {
      do {
        ret = adc2_get_raw(CONFIG_MQ_7b_ADC_CHANNEL_A, ADC_WIDTH_BIT_DEFAULT,
                           &adc_raw[0][0]);
        ret2 = adc2_get_raw(CONFIG_MQ_7b_ADC_CHANNEL_B, ADC_WIDTH_BIT_DEFAULT,
                            &adc_raw[1][0]);
