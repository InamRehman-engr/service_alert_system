// normal values are in range of 500mV using 10k resister
#include "MP_5.h"
...
esp_err_t ADC_init(void) {
  ...
  if (CONFIG_MP_5_ADC_UNIT == 1)
  // ADC1 config
  {
    ESP_ERROR_CHECK(adc1_config_width(ADC_WIDTH_BIT_DEFAULT));
    ESP_ERROR_CHECK(adc1_config_channel_atten(CONFIG_MP_5_ADC_CHANNEL_A,
                                              ADC_EXAMPLE_ATTEN));
    ESP_ERROR_CHECK(adc1_config_channel_atten(CONFIG_MP_5_ADC_CHANNEL_B,
                                              ADC_EXAMPLE_ATTEN));
  } else {
    // ADC2 config
    ESP_ERROR_CHECK(adc2_config_channel_atten(CONFIG_MP_5_ADC_CHANNEL_A,
                                              ADC_EXAMPLE_ATTEN));
    ESP_ERROR_CHECK(adc2_config_channel_atten(CONFIG_MP_5_ADC_CHANNEL_B,
                                              ADC_EXAMPLE_ATTEN));
  }
  return ESP_OK;
}
void get_MP_5(void) {
  ...
  for (int a = 0; a < sample; a++) {
    if (CONFIG_MP_5_ADC_UNIT == 1) {
      adc_raw[0][0] = adc1_get_raw(CONFIG_MP_5_ADC_CHANNEL_A);
      adc_raw[1][0] = adc1_get_raw(CONFIG_MP_5_ADC_CHANNEL_B);
    } else {
      do {
        ret = adc2_get_raw(CONFIG_MP_5_ADC_CHANNEL_A, ADC_WIDTH_BIT_DEFAULT,
                           &adc_raw[0][0]);
        ret2 = adc2_get_raw(CONFIG_MP_5_ADC_CHANNEL_B, ADC_WIDTH_BIT_DEFAULT,
                            &adc_raw[1][0]);
