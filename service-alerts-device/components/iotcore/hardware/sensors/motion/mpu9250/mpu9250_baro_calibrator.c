...
static esp_err_t
mpu9250_baro_cal_discard_messages(mpu9250_handle_t mpu9250_handle,
                                  uint16_t samples) {
...
static esp_err_t mpu9250_baro_cal_calc_means(mpu9250_handle_t mpu9250_handle,
                                             uint8_t cycles) {
...
static esp_err_t mpu9250_baro_cal_calc_var(mpu9250_handle_t mpu9250_handle,
                                           uint8_t cycles) {
  uint64_t baro_sum_pressure = 0;
  uint64_t baro_sum_temperature = 0;
  ...
  for (uint8_t i = 0; i < cycles; i++) {
    ...
    printf("BMP388::mpu9250_baro_cal_calc_var SUM P[%d] T[%d]\n",
           (uint32_t)(baro_sum_pressure / cycles),
           (uint32_t)(baro_sum_temperature / cycles));
  }
...
static esp_err_t mpu9250_baro_cal_calc_sqm(mpu9250_handle_t mpu9250_handle) {
  mpu9250_handle->baro.cal.kalman_pressure.sqm =
      sqrt(mpu9250_handle->baro.cal.kalman_pressure.variance);
  mpu9250_handle->baro.cal.kalman_temperature.sqm =
      sqrt(mpu9250_handle->baro.cal.kalman_temperature.variance);
...
static esp_err_t mpu9250_baro_cal_calc_bias(mpu9250_handle_t mpu9250_handle) {
  // Calc Bias
  {
    ...
    ESP_ERROR_CHECK(mpu9250_baro_cal_calc_var(
        mpu9250_handle, MPU9250_BARO_CAL_MAX_KSAMPLE_CYCLES));
...
esp_err_t mpu9250_baro_calibrate(mpu9250_handle_t mpu9250_handle) {
  ...
  ESP_ERROR_CHECK(mpu9250_baro_cal_calc_means(
      mpu9250_handle, MPU9250_BARO_CAL_MAX_KSAMPLE_CYCLES));
