...
static esp_err_t mpu9250_mag_cal_init(mpu9250_handle_t mpu9250_handle,
                                      uint8_t precision) {
  ESP_ERROR_CHECK(
      mpu9250_mag_set_mode2_with_precision(mpu9250_handle, precision));
  ESP_ERROR_CHECK(mpu9250_mag_set_continuous_reading(mpu9250_handle));
  return ESP_OK;
}
static esp_err_t
mpu9250_mag_cal_discard_messages(mpu9250_handle_t mpu9250_handle,
                                 uint16_t samples) {
...
static esp_err_t mpu9250_mag_cal_calc_means(mpu9250_handle_t mpu9250_handle,
                                            uint8_t cycles) {
  ...
  for (uint8_t i = 0; i < 3; i++) {
    mpu9250_handle->mag.cal.means[mpu9250_handle->mag.precision].array[i] =
        mag_sum[i] / (cycles * 1000);
  }
  return ESP_OK;
}

static esp_err_t mpu9250_mag_cal_calc_var(mpu9250_handle_t mpu9250_handle,
                                          uint8_t cycles) {
    ...
    for (uint8_t i = 0; i < 3; i++) {
      mag_sum[i] +=
          (mpu9250_handle->raw_data.data_s_vector.mag[i] -
           mpu9250_handle->mag.cal.means[mpu9250_handle->mag.precision]
               .array[i]) *
          (mpu9250_handle->raw_data.data_s_vector.mag[i] -
           mpu9250_handle->mag.cal.means[mpu9250_handle->mag.precision]
               .array[i]);
  ...
  for (uint8_t i = 0; i < 3; i++) {
    mpu9250_handle->mag.cal.var[mpu9250_handle->mag.precision].array[i] =
        (uint16_t)(mag_sum[i] / (cycles * 1000));
  }
...
static esp_err_t mpu9250_mag_cal_calc_sqm(mpu9250_handle_t mpu9250_handle) {
  for (uint8_t i = 0; i < 3; i++) {
    mpu9250_handle->mag.cal.sqm[mpu9250_handle->mag.precision].array[i] = sqrt(
        mpu9250_handle->mag.cal.var[mpu9250_handle->mag.precision].array[i]);
  }
...
static esp_err_t mpu9250_mag_cal_calc_bias(mpu9250_handle_t mpu9250_handle) {
  // Calc Bias
  {
    ...
    memset(mpu9250_handle->mag.cal.var[mpu9250_handle->mag.precision].array, 0,
           sizeof(mpu9250_handle->mag.cal.var[mpu9250_handle->mag.precision]
                      .array)); // Zero out var
    memset(mpu9250_handle->mag.cal.sqm[mpu9250_handle->mag.precision].array, 0,
           sizeof(mpu9250_handle->mag.cal.sqm[mpu9250_handle->mag.precision]
                      .array)); // Zero out sqm

    ESP_ERROR_CHECK(mpu9250_mag_cal_calc_var(
        mpu9250_handle, MPU9250_MAG_CAL_MAX_KSAMPLE_CYCLES));
...
esp_err_t mpu9250_mag_calibrate(mpu9250_handle_t mpu9250_handle) {
  ESP_ERROR_CHECK(
      mpu9250_mag_cal_init(mpu9250_handle, INV_MAG_PRECISION_14_BITS));
  ESP_ERROR_CHECK(mpu9250_mag_cal_discard_messages(mpu9250_handle, 2000));
  ESP_ERROR_CHECK(mpu9250_mag_cal_calc_means(
      mpu9250_handle, MPU9250_MAG_CAL_MAX_KSAMPLE_CYCLES));
  ...
  ESP_ERROR_CHECK(
      mpu9250_mag_cal_init(mpu9250_handle, INV_MAG_PRECISION_16_BITS));
  ESP_ERROR_CHECK(mpu9250_mag_cal_discard_messages(mpu9250_handle, 2000));
  ESP_ERROR_CHECK(mpu9250_mag_cal_calc_means(
      mpu9250_handle, MPU9250_MAG_CAL_MAX_KSAMPLE_CYCLES));
  ESP_ERROR_CHECK(mpu9250_mag_cal_calc_bias(mpu9250_handle));
