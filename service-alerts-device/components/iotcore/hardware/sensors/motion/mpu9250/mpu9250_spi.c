...
esp_err_t mpu9250_read8(mpu9250_handle_t mpu9250_handle, uint8_t reg,
                        uint8_t *val) {
  esp_err_t ret;
  spi_transaction_t t;
  ...
  ret = spi_device_polling_transmit(
      ((mpu9250_handle_t)mpu9250_handle)->device_handle, &t); // Transmit!
  if (ret == ESP_OK) {
    *val = t.rx_data[0];
  }
  return ret;
...
esp_err_t mpu9250_write8(mpu9250_handle_t mpu9250_handle, uint8_t reg,
                         uint8_t val) {
  esp_err_t ret;
  spi_transaction_t t;
  memset(&t, 0, sizeof(t)); // Zero out the transaction
...
esp_err_t mpu9250_read_buff(mpu9250_handle_t mpu9250_handle, uint8_t reg,
...
esp_err_t mpu9250_write_buff(mpu9250_handle_t mpu9250_handle, uint8_t reg,
                             void *buff, uint8_t length) {
  esp_err_t ret;
  spi_transaction_t t;
  memset(&t, 0, sizeof(t)); // Zero out the transaction
...
esp_err_t mpu9250_spi_init(mpu9250_handle_t mpu9250_handle) {
  ...
  mpu9250_handle->devcfg.clock_speed_hz =
      SPI_MASTER_FREQ_20M; // Clock out at 20 MHz
  mpu9250_handle->devcfg.address_bits = 8;
  mpu9250_handle->devcfg.mode = 3; // SPI mode 3
  mpu9250_handle->devcfg.queue_size =
      7; // We want to be able to queue 7 transactions at a time

  mpu9250_handle->int_pin = PIN_NUM_INT;

  ESP_LOGI(TAG, "miso  [%d]", mpu9250_handle->buscfg.miso_io_num);
  ESP_LOGI(TAG, "mosi  [%d]", mpu9250_handle->buscfg.mosi_io_num);
  ESP_LOGI(TAG, "clk   [%d]", mpu9250_handle->buscfg.sclk_io_num);
  ESP_LOGI(TAG, "cs    [%d]", mpu9250_handle->devcfg.spics_io_num);
  ESP_LOGI(TAG, "freq  [%d]", mpu9250_handle->devcfg.clock_speed_hz);
  ESP_LOGI(TAG, "flags [%d]", mpu9250_handle->devcfg.flags);

  // Initialize the SPI bus
  ESP_ERROR_CHECK(
      spi_bus_initialize(MY_SPI_MPU9250_HOST, &mpu9250_handle->buscfg, 0));
  // Attach the MPU9250 to the SPI bus
  ESP_ERROR_CHECK(spi_bus_add_device(MY_SPI_MPU9250_HOST,
                                     &mpu9250_handle->devcfg,
                                     &(mpu9250_handle->device_handle)));
