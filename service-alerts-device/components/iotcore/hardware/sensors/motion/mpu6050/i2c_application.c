...
void vI2CWrite(void *pvParameters) {
  ...
  /* The queue is created xStaticReadQueueto hold a maximum of 1 structure
   * entry. */
  xQueueI2CWriteBuffer =
      xQueueCreateStatic(QUEUE_LENGTH, WRITE_ITEM_SIZE, ucQueueWriteStorageArea,
                         &xStaticWriteQueue);
  ...
  xBinarySemaphoreI2CAppEndOfWrite =
      xSemaphoreCreateBinaryStatic(&xSemaphoreWriteBuffer);
  ...
  if (xBinarySemaphoreI2CAppEndOfWrite != NULL) {
    /* As per most tasks, this task is implemented within an infinite loop.
            Take the semaphore once to start with so the semaphore is empty
       before the infinite loop is entered.  The semaphore was created before
  ...
  for (;;) {
    ...
    xStatus = xQueueReceive(xQueueI2CWriteBuffer, (void *)&lReceivedValue,
                            portMAX_DELAY);
    if (xStatus == pdPASS) {
      /* Write on I2C */
      ...
      vTaskDelay(GUARD_CYCLE / portTICK_PERIOD_MS);
...
void vI2CRead(void *pvParameters) {
  ...
  xQueueI2CReadBuffer = xQueueCreateStatic(
      QUEUE_LENGTH, READ_ITEM_SIZE, ucQueueReadStorageArea, &xStaticReadQueue);

  /* Create a binary semaphore without using any dynamic memory
  allocation.  The semaphore's data structures will be saved into
  the xSemaphoreReadBuffer variable. */
  xBinarySemaphoreI2CAppEndOfRead =
      xSemaphoreCreateBinaryStatic(&xSemaphoreReadBuffer);
  ...
  for (;;) {
    ...
    task should remain in the Blocked state to wait for data to be available
    should the queue already be empty. */
    xStatus = xQueueReceive(xQueueI2CReadBuffer, (void *)&lReceivedValue,
                            portMAX_DELAY);
    if (xStatus == pdPASS) {
      /* Read from I2C */
