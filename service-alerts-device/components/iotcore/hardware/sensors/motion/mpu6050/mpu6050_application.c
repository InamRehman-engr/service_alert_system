...
static angle_calculations pitch_angle_calculations = {.angle = 0.0,
                                                      .gyro_y = 0.0,
                                                      .q_bias = 0.0,
                                                      .angle_err = 0.0,
                                                      .q_angle = 0.1,
                                                      ...
                                                      .pdot = {0, 0, 0, 0},
                                                      .pp = {{1, 0}, {0, 1}}};
static angle_calculations roll_angle_calculations = {.angle = 0.0,
                                                     .gyro_y = 0.0,
                                                     .q_bias = 0.0,
                                                     .angle_err = 0.0,
                                                     .q_angle = 0.1,
...
void vMPU6050Task(void *pvParameters) {
  ...
  for (;;) {
    ...
    vTaskDelay(ACCELEROMETER_READING_FREQUENCY_MS / portTICK_PERIOD_MS);
  }
...
uint8_t uMPU6050ReadAccelerometer() {
  ...
  accelration_x =
      ((uint16_t)(mpu6050Buffer[0] << 8)) | ((uint16_t)(mpu6050Buffer[1]));

  accelration_y =
      ((uint16_t)(mpu6050Buffer[2] << 8)) | ((uint16_t)(mpu6050Buffer[3]));

  accelration_z =
      ((uint16_t)(mpu6050Buffer[4] << 8)) | ((uint16_t)(mpu6050Buffer[5]));
...
uint8_t uMPU6050ReadGyroscope() {
  ...
  gyro_x = ((uint16_t)(mpu6050Buffer[0] << 8)) | ((uint16_t)(mpu6050Buffer[1]));

  gyro_y = ((uint16_t)(mpu6050Buffer[2] << 8)) | ((uint16_t)(mpu6050Buffer[3]));

  gyro_z = ((uint16_t)(mpu6050Buffer[4] << 8)) | ((uint16_t)(mpu6050Buffer[5]));
...
/**
 * @brief  Get Direction of the MPU 6050, whether it's faced upside down or
 * right position.
 * @param  None
...
uint8_t uGetDirection() {
  ...
  if (angle_pitch > THRESHOLD || angle_pitch < THRESHOLD_NEG ||
      angle_roll > THRESHOLD || angle_roll < THRESHOLD_NEG) {
    status = 1;
  } else {
    // angles of pitch and rotation are almost zero, check whether flipped 180
    // degree or 0 degree
    if (a_z > 0) {
      // flipped 180 degree
      status = 2;
...
void uMPU6050Calibration() {
  ...
  if (calibration_mode == CALIBRATION_MODE_OFF) {
    ...
    ESP_LOGI("mpu6050", "Place the accelerometer on it's front end and wait "
                        "util calibration is done");
    vTaskDelay(POSITIONING_DELAY / portTICK_PERIOD_MS);

    for (i = 0; i < NUMBER_READINGS_CALIBRATION +
                        NUMBER_READINGS_CALIBRATION_DISCARDED;
         i++) {
    ...
    ESP_LOGI("mpu6050", "Mean Acc: ( %.3ld, %.3ld, %.3ld)", total_ax_mean,
             total_ay_mean, total_az_mean);
    ...
    /************ Front Side  ************/
    ESP_LOGI("mpu6050",
             "Place the accelerometer on it's front end and donot touch it");
    vTaskDelay(POSITIONING_DELAY / portTICK_PERIOD_MS);
    ...
    /************ Back Side  ************/
    ESP_LOGI("mpu6050",
             "Place the accelerometer on it's back end and donot touch it");
    vTaskDelay(POSITIONING_DELAY / portTICK_PERIOD_MS);
    ...
    ESP_LOGI(
        "mpu6050",
        "Place the accelerometer on it's portrait up side and donot touch it");
    vTaskDelay(POSITIONING_DELAY / portTICK_PERIOD_MS);
    ...
    /************ Portrait Down Side  ************/
    ESP_LOGI("mpu6050", "Place the accelerometer on it's portrait down side "
                        "and donot touch it");
    vTaskDelay(POSITIONING_DELAY / portTICK_PERIOD_MS);
    ...
    /************ Landscape Left Side  ************/
    ESP_LOGI("mpu6050", "Place the accelerometer on it's landscape left side "
                        "and donot touch it");
    vTaskDelay(POSITIONING_DELAY / portTICK_PERIOD_MS);
    ...
    ESP_LOGI("mpu6050", "Place the accelerometer on it's landscape right side "
                        "and donot touch it");
    vTaskDelay(POSITIONING_DELAY / portTICK_PERIOD_MS);
  ...
  ESP_LOGI("mpu6050", "Cal Acc: ( %.3d, %.3d, %.3d)", cal_a_x, cal_a_y,
           cal_a_z);
}
