...
#define ICM20602_INIT()                                                        \
  {                                                                            \
    .id = 0, .hal_wr = NULL, .hal_rd = NULL, .hal_sleep = NULL,                \
    .mutex_lock = NULL, .mutex_unlock = NULL, .use_accel = false,              \
    .accel_fifo = false, .accel_dlpf = 0, .accel_g = 0, .use_gyro = false,     \
    .gyro_fifo = false, .gyro_dlpf = 0, .gyro_dps = 0, .sample_rate_div = 1,   \
    .i2c_disable = false,                                                      \
  }
...
#define ICM20602_DEFAULT_INIT()                                                \
  {                                                                            \
    .id = 0, .hal_wr = NULL, .hal_rd = NULL, .hal_sleep = NULL,                \
    .mutex_lock = NULL, .mutex_unlock = NULL, .use_accel = true,               \
    .accel_fifo = false, .accel_dlpf = ICM20602_ACCEL_DLPF_10_2_HZ,            \
    .accel_g = ICM20602_ACCEL_RANGE_4G, .use_gyro = true, .gyro_fifo = false,  \
    .gyro_dlpf = ICM20602_GYRO_DLPF_10_HZ,                                     \
    .gyro_dps = ICM20602_GYRO_RANGE_2000_DPS, .sample_rate_div = 100,          \
    .i2c_disable = false,                                                      \
  }
 ...
 * \return zero on success, anything else is an error
 */
extern int8_t icm20602_init(struct icm20602_dev *dev);
 ...
 * \return zero on success, anything else is an error
 */
extern int8_t icm20602_read_accel(struct icm20602_dev *dev, float *p_x,
                                  float *p_y, float *p_z);
 ...
 * \return zero on success, anything else is an error
 */
extern int8_t icm20602_read_gyro(struct icm20602_dev *dev, float *p_x,
                                 float *p_y, float *p_z);
 ...
 * \return zero on success, anything else is an error
 */
extern int8_t icm20602_read_data(struct icm20602_dev *dev, float *p_ax,
 ...
 * \return zero on success, anything else is an error
 */
extern int8_t icm20602_read_accel_raw(struct icm20602_dev *dev, int16_t *p_x,
 ...
 * \return zero on success, anything else is an error
 */
extern int8_t icm20602_read_gyro_raw(struct icm20602_dev *dev, int16_t *p_x,
                                     int16_t *p_y, int16_t *p_z);
 ...
 * \return zero on success, anything else is an error
 */
extern int8_t icm20602_read_data_raw(struct icm20602_dev *dev, int16_t *p_ax,
                                     int16_t *p_ay, int16_t *p_az,
                                     int16_t *p_gx, int16_t *p_gy,
 ...
 * \return zero on success, anything else is an error
 */
extern int8_t icm20602_read_accel_fifo(struct icm20602_dev *dev, float *x,
                                       float *y, float *z);
 ...
 * \return zero on success, anything else is an error
 */
extern int8_t icm20602_read_gyro_fifo(struct icm20602_dev *dev, float *x,
                                      float *y, float *z);
 ...
 * \return zero on success, anything else is an error
 */
extern int8_t icm20602_read_fifo_data(struct icm20602_dev *dev, float *p_ax,
                                      float *p_ay, float *p_az, float *p_gx,
                                      float *p_gy, float *p_gz, float *p_t);
 ...
 * \return zero on success, anything else is an error
 */
extern int8_t icm20602_read_fifo_accel_raw(struct icm20602_dev *dev,
 ...
 * \return zero on success, anything else is an error
 */
extern int8_t icm20602_read_fifo_gyro_raw(struct icm20602_dev *dev,
                                          int16_t *p_x, int16_t *p_y,
                                          int16_t *p_z);
 ...
 * \return zero on success, anything else is an error
 */
extern int8_t icm20602_read_fifo_data_raw(struct icm20602_dev *dev,
                                          int16_t *p_ax, int16_t *p_ay,
                                          int16_t *p_az, int16_t *p_gx,
                                          int16_t *p_gy, int16_t *p_gz,
                                          int16_t *p_t);
