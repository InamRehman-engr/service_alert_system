...
i2c_functions *i2c_func_obj;
/***** Macros *****/
int8_t i2c_write(uint8_t id, uint8_t reg, uint8_t *data, uint16_t len) {
  uint8_t buffer[len + 1];
  buffer[0] = reg;
  memcpy(buffer + 1, data, len);
  return i2c_func_obj->i2c_send(id, buffer, sizeof(buffer),
                                i2c_func_obj->device);
}
int8_t i2c_read(uint8_t id, uint8_t reg, uint8_t *data, uint16_t len) {
  int8_t ret = i2c_func_obj->i2c_send_receive(id, &reg, 1, data, len,
                                              i2c_func_obj->device);
  return ret;
}
void _sleep(uint32_t ms) { vTaskDelay(pdMS_TO_TICKS(ms)); }
...
int8_t _read_data(struct icm20602_dev *dev, uint8_t reg, uint8_t *buf,
                  uint32_t len) {
  int8_t r = 0;
  if (i2c_func_obj->device == NULL) {
    return false;
  }
  if (dev->mutex_lock) {
...
int8_t icm20602_init(struct icm20602_dev *dev) {
  uint8_t tmp = 0;
  int8_t r = 0;
  i2c_func_obj = malloc(sizeof(i2c_functions));
  memcpy(i2c_func_obj, dev->i2c_obj, sizeof(i2c_functions));
  // if ((!i2c_write) || (!i2c_read) || (!_sleep)) {
  //   return false;
  // }
  if (i2c_func_obj->device == NULL) {
    return false;
  ...
  // full reset of chip
  tmp = 0x80;
  r = i2c_write(dev->id, REG_PWR_MGMT_1, &tmp, 1);
  ...
  r = i2c_read(dev->id, REG_WHO_AM_I, &tmp, 1);
  ON_ERROR_GOTO((0 == r), return_err);
  ...
  tmp = 0x3F;
  r = i2c_write(dev->id, REG_PWR_MGMT_2, &tmp, 1);
  ON_ERROR_GOTO((0 == r), return_err);

  // disable fifo
  tmp = 0x00;
  r = i2c_write(dev->id, REG_USER_CTRL, &tmp, 1);
  ON_ERROR_GOTO((0 == r), return_err);

  if (dev->i2c_disable) {
    // disable chip I2C communications
    tmp = 0x40;
    r = i2c_write(dev->id, REG_I2C_IF, &tmp, 1);
    ON_ERROR_GOTO((0 == r), return_err);
  }
  ...
  if (dev->use_accel) {
    if (ICM20602_ACCEL_DLPF_BYPASS_1046_HZ == dev->accel_dlpf) {
      tmp = (1 << 3);
      r = i2c_write(dev->id, REG_ACCEL_CONFIG_2, &tmp, 1);
      ON_ERROR_GOTO((0 == r), return_err);
    } else {
      tmp = dev->accel_dlpf;
      r = i2c_write(dev->id, REG_ACCEL_CONFIG_2, &tmp, 1);
    ...
    tmp = (dev->accel_g) << 2;
    r = i2c_write(dev->id, REG_ACCEL_CONFIG, &tmp, 1);
    ON_ERROR_GOTO((0 == r), return_err);
  }
  ...
  if (dev->use_gyro) {
    if (ICM20602_GYRO_DLPF_BYPASS_3281_HZ == dev->gyro_dlpf) {
      ...
      tmp = 0x00;
      r = i2c_write(dev->id, REG_CONFIG, &tmp, 1);
      ON_ERROR_GOTO((0 == r), return_err);

      tmp = (dev->gyro_dps << 3) | 0x02; // see table page 37 of datasheet
      r = i2c_write(dev->id, REG_GYRO_CONFIG, &tmp, 1);
      ON_ERROR_GOTO((0 == r), return_err);
  ...
  r = i2c_write(dev->id, REG_FIFO_EN, &tmp, 1);
  ON_ERROR_GOTO((0 == r), return_err);
  ...
  r = i2c_write(dev->id, REG_SMPLRT_DIV, &tmp, 1);
  ON_ERROR_GOTO((0 == r), return_err);
...
int8_t icm20602_read_accel(struct icm20602_dev *dev, float *p_x, float *p_y,
                           float *p_z) {
...
int8_t icm20602_read_gyro(struct icm20602_dev *dev, float *p_x, float *p_y,
                          float *p_z) {
...
int8_t icm20602_read_data(struct icm20602_dev *dev, float *p_ax, float *p_ay,
                          float *p_az, float *p_gx, float *p_gy, float *p_gz,
                          float *p_t) {
  float accel_sensitivity;
  float gyro_sensitivity;
...
int8_t icm20602_read_accel_raw(struct icm20602_dev *dev, int16_t *p_x,
...
int8_t icm20602_read_gyro_raw(struct icm20602_dev *dev, int16_t *p_x,
                              int16_t *p_y, int16_t *p_z) {
  uint8_t buf[6] = {0};
  int8_t r = 0;
...
int8_t icm20602_read_data_raw(struct icm20602_dev *dev, int16_t *p_ax,
                              int16_t *p_ay, int16_t *p_az, int16_t *p_gx,
                              int16_t *p_gy, int16_t *p_gz, int16_t *p_t) {
  uint8_t buf[14] = {0};
  int8_t r = 0;
...
int8_t icm20602_read_accel_fifo(struct icm20602_dev *dev, float *p_x,
                                float *p_y, float *p_z) {
  float accel_sensitivity;
  int16_t x, y, z;
  int8_t r = 0;
...
int8_t icm20602_read_gyro_fifo(struct icm20602_dev *dev, float *p_x, float *p_y,
                               float *p_z) {
  float gyro_sensitivity;
  int16_t x, y, z;
  int8_t r = 0;
...
int8_t icm20602_read_fifo_data(struct icm20602_dev *dev, float *p_ax,
                               float *p_ay, float *p_az, float *p_gx,
                               float *p_gy, float *p_gz, float *p_t) {
  float accel_sensitivity;
  float gyro_sensitivity;
  int16_t ax, ay, az, gx, gy, gz, t;
  int8_t r = 0;
...
int8_t icm20602_read_fifo_accel_raw(struct icm20602_dev *dev, int16_t *p_x,
                                    int16_t *p_y, int16_t *p_z) {
  uint8_t buf[6] = {0};
  int8_t r = 0;
...
int8_t icm20602_read_fifo_gyro_raw(struct icm20602_dev *dev, int16_t *p_x,
                                   int16_t *p_y, int16_t *p_z) {
  uint8_t buf[6] = {0};
  int8_t r = 0;
...
int8_t icm20602_read_fifo_data_raw(struct icm20602_dev *dev, int16_t *p_ax,
                                   int16_t *p_ay, int16_t *p_az, int16_t *p_gx,
                                   int16_t *p_gy, int16_t *p_gz, int16_t *p_t) {
  uint8_t buf[14] = {0};
  int8_t r = 0;
