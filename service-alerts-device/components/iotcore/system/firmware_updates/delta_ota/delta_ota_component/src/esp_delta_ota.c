...
static int esp_delta_ota_write_cb(void *arg_p, const uint8_t *buf_p,
                                  size_t size) {
  if (size <= 0) {
  ...
  if (!handle->user_data) {
    ...
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Error in write_cb_with_user_data(): %s",
               esp_err_to_name(err));
      return ESP_FAIL;
    }
  }
...
esp_delta_ota_handle_t esp_delta_ota_init(esp_delta_ota_cfg_t *cfg) {
  ...
  int ret = detools_apply_patch_init(ctx->apply_patch, &esp_delta_ota_read_cb,
                                     &esp_delta_ota_seek_cb, 0,
                                     &esp_delta_ota_write_cb, ctx);
  if (ret < 0) {
    ESP_LOGE(TAG, "Error while initializing delta_ota: %s",
             detools_error_as_string(ret));
...
esp_err_t esp_delta_ota_feed_patch(esp_delta_ota_handle_t handle,
  ...
  int err =
      detools_apply_patch_process(ctx->apply_patch, (const uint8_t *)buf, size);
  if (err != 0) {
    ESP_LOGE(TAG, "Error while applying patch: %s",
             detools_error_as_string(err));
    return ESP_FAIL;
  }
  return ESP_OK;
}
...
esp_err_t esp_delta_ota_finalize(esp_delta_ota_handle_t handle) {
  ...
  if (err < 0) {
    ESP_LOGE(TAG, "Error while finishing the patching: %s",
             detools_error_as_string(err));
    return ESP_FAIL;
  }
  return ESP_OK;
