...
// Callback for working on the data generated by applying patch on the source
// data
typedef esp_err_t (*merged_stream_write_cb_t)(const uint8_t *buf_p,
                                              size_t size);
typedef esp_err_t (*merged_stream_write_cb_with_user_ctx_t)(
    const uint8_t *buf_p, size_t size, void *user_data);

typedef struct esp_delta_ota_cfg {
  void *user_data;       /*!< User Data */
  src_read_cb_t read_cb; /*!< Read Callback */
  union {
    merged_stream_write_cb_with_user_ctx_t
        write_cb_with_user_data; /*!< Write Callback with user data */
    merged_stream_write_cb_t write_cb
        DEPRECATED_ATTRIBUTE; /*!< Write Callback */
  };
} esp_delta_ota_cfg_t;
...
/**
 * @brief This function performs the patch applying operation on the source
 * data.
 *
