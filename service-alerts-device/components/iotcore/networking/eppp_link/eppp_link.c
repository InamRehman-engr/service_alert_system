...
static esp_err_t transmit(void *h, void *buffer, size_t len) {
  ...
  do { // TODO(IDF-9194): Refactor this loop to allocate only once and perform
    //       fragmentation after receiving from the queue (applicable only if
    //       MTU > MAX_PAYLOAD)
...
static esp_netif_t *netif_init(eppp_type_t role, eppp_config_t *eppp_config) {
  if (s_eppp_netif_count > 9) { // Limit to max 10 netifs, since we use "EPPPx"
                                // as the unique key (where x is 0-9)
...
#if CONFIG_EPPP_LINK_DEVICE_SPI
  h->out_queue =
      xQueueCreate(CONFIG_EPPP_LINK_PACKET_QUEUE_SIZE, sizeof(struct packet));
  if (!h->out_queue) {
  ...
  if (role == EPPP_SERVER) {
    esp_timer_create_args_t args = {
        .callback = &timer_callback, .arg = h, .name = "timer"};
  ...
  if_key[sizeof(if_key) - 2 /* 2 = two chars before the terminator */] +=
      s_eppp_netif_count++;
  base_netif_cfg.if_key = if_key;
  if (eppp_config->ppp.netif_description) {
    base_netif_cfg.if_desc = eppp_config->ppp.netif_description;
  } else {
    base_netif_cfg.if_desc =
        role == EPPP_CLIENT ? "pppos_client" : "pppos_server";
  }
  ...
  esp_netif_config_t netif_ppp_config = {.base = &base_netif_cfg,
                                         ...
                                         .stack =
                                             ESP_NETIF_NETSTACK_DEFAULT_PPP};
...
static void on_ppp_event(void *arg, esp_event_base_t base, int32_t event_id,
                         void *data) {
...
#define NEXT_TRANSACTION_SIZE(a, b)                                            \
  (((a) > (b)) ? (a) : (b)) /* next transaction: whichever is bigger */
...
static esp_err_t deinit_master(esp_netif_t *netif) {
  struct eppp_handle *h = esp_netif_get_io_driver(netif);
  ESP_RETURN_ON_ERROR(spi_bus_remove_device(h->spi_device), TAG,
                      "Failed to remove SPI bus");
  ESP_RETURN_ON_ERROR(spi_bus_free(h->spi_host), TAG, "Failed to free SPI bus");
  return ESP_OK;
}

static esp_err_t init_master(struct eppp_config_spi_s *config,
                             esp_netif_t *netif) {
  struct eppp_handle *h = esp_netif_get_io_driver(netif);
  ...
  gpio_isr_handler_add(config->intr, gpio_isr_handler,
...
static void post_setup(spi_slave_transaction_t *trans) {
  ...
  if (h->transaction_size == 0) { // If no transaction planned:
    if (h->outbound.len == 0) {   // we're blocked if we don't have any data
      ...
      h->blocked =
          SLAVE_WANTS_WRITE; // we notify the master that we want to write
...
static esp_err_t deinit_slave(esp_netif_t *netif) {
  struct eppp_handle *h = esp_netif_get_io_driver(netif);
  ESP_RETURN_ON_ERROR(spi_slave_free(h->spi_host), TAG,
                      "Failed to free SPI slave host");
  ESP_RETURN_ON_ERROR(spi_bus_remove_device(h->spi_device), TAG,
...
static esp_err_t init_slave(struct eppp_config_spi_s *config,
                            esp_netif_t *netif) {
  struct eppp_handle *h = esp_netif_get_io_driver(netif);
  h->spi_host = config->host;
  h->gpio_intr = config->intr;
...
typedef esp_err_t (*perform_transaction_t)(struct eppp_handle *h, size_t len,
                                           const void *tx_buffer,
                                           void *rx_buffer);

static esp_err_t perform_transaction_master(struct eppp_handle *h, size_t len,
                                            const void *tx_buffer,
                                            void *rx_buffer) {
  spi_transaction_t t = {};
  t.length = len * 8;
...
static esp_err_t perform_transaction_slave(struct eppp_handle *h, size_t len,
                                           const void *tx_buffer,
                                           void *rx_buffer) {
...
esp_err_t eppp_perform(esp_netif_t *netif) {
  ...
  const perform_transaction_t perform_transaction =
      h->role == EPPP_CLIENT ? perform_transaction_master
                             : perform_transaction_slave;
  ...
  if (h->role == EPPP_CLIENT) {
    ...
    if (h->outbound.len == 0 && h->transaction_size == 0 &&
        h->blocked == NONE) {
  ...
  if (h->outbound.len <= h->transaction_size && allow_test_tx == false) {
    ...
    if (h->outbound.len > 0) {
      memcpy(out_buf + sizeof(struct header), h->outbound.data,
             h->outbound.len);
      free(h->outbound.data);
      ESP_LOG_BUFFER_HEXDUMP(TAG, out_buf + sizeof(struct header), head->size,
                             ESP_LOG_VERBOSE);
      h->outbound.data = NULL;
      h->outbound.len = 0;
    }
    ...
    // outbound is bigger, need to transmit in another transaction (keep this
    // empty)
    head->size = 0;
  }
  ...
  head->check =
      esp_rom_crc16_le(0, out_buf, sizeof(struct header) - sizeof(uint16_t));
  esp_err_t ret = perform_transaction(
      h, sizeof(struct header) + h->transaction_size, out_buf, in_buf);
  if (ret != ESP_OK) {
  ...
  head = (void *)in_buf;
  uint16_t check =
      esp_rom_crc16_le(0, in_buf, sizeof(struct header) - sizeof(uint16_t));
  ...
  if (head->size > 0) {
    ESP_LOG_BUFFER_HEXDUMP(TAG, in_buf + sizeof(struct header), head->size,
                           ESP_LOG_VERBOSE);
...
static esp_err_t init_uart(struct eppp_handle *h, eppp_config_t *config) {
  h->uart_port = config->transport_config.uart.port;
  uart_config_t uart_config = {};
  uart_config.baud_rate = config->transport_config.uart.baud;
  ...
  ESP_RETURN_ON_ERROR(
      uart_driver_install(
          h->uart_port, config->transport_config.uart.rx_buffer_size, 0,
          config->transport_config.uart.queue_size, &h->uart_event_queue, 0),
      TAG, "Failed to install UART");
  ESP_RETURN_ON_ERROR(uart_param_config(h->uart_port, &uart_config), TAG,
                      "Failed to set params");
  ESP_RETURN_ON_ERROR(uart_set_pin(h->uart_port,
                                   config->transport_config.uart.tx_io,
                                   config->transport_config.uart.rx_io,
...
esp_netif_t *eppp_open(eppp_type_t role, eppp_config_t *config,
  ...
  if (config->task.run_task == false) {
    ESP_LOGE(TAG, "task.run_task == false is invalid in this API. Please use "
                  "eppp_init()");
    return NULL;
  }
  ...
  if (xTaskCreate(ppp_task, "ppp connect", config->task.stack_size, netif,
                  config->task.priority, NULL) != pdTRUE) {
...
eppp_config_t get_eppp_config(eppp_type_t role) {
  eppp_config_t config = {0};
#ifdef CONFIG_EPPP_LINK_DEVICE_UART
  config.transport = EPPP_TRANSPORT_UART;
  config.transport_config.uart.baud = CONFIG_EPPP_UART_BAUD_RATE;
  config.transport_config.uart.port = CONFIG_EPPP_UART_PORT;
  config.transport_config.uart.queue_size = CONFIG_EPPP_UART_QUEUE_SIZE;
  config.transport_config.uart.rx_buffer_size = CONFIG_EPPP_UART_RX_BUFFER_SIZE;
  config.transport_config.uart.rx_io = CONFIG_EPPP_UART_RX_IO;
  config.transport_config.uart.tx_io = CONFIG_EPPP_UART_TX_IO;
#elif defined(CONFIG_EPPP_LINK_DEVICE_SPI)
  config.transport = EPPP_TRANSPORT_SPI;
  config.transport_config.spi.host = CONFIG_EPPP_SPI_HOST;
  config.transport_config.spi.mosi = CONFIG_EPPP_SPI_MOSI_IO;
  config.transport_config.spi.miso = CONFIG_EPPP_SPI_MISO_IO;
  config.transport_config.spi.sclk = CONFIG_EPPP_SPI_SCLK_IO;
  config.transport_config.spi.cs = CONFIG_EPPP_SPI_CS_IO;
  config.transport_config.spi.intr = CONFIG_EPPP_SPI_INTR_IO;
  config.transport_config.spi.freq = CONFIG_EPPP_SPI_FREQ;
  config.transport_config.spi.input_delay_ns = 0;
  config.transport_config.spi.cs_ena_pretrans = 0;
  config.transport_config.spi.cs_ena_posttrans = 0;
#elif defined(CONFIG_EPPP_LINK_DEVICE_SDIO)
  config.transport = EPPP_TRANSPORT_SDIO;
  config.transport_config.sdio.width = CONFIG_EPPP_SDIO_WIDTH;
  config.transport_config.sdio.clk = CONFIG_EPPP_SDIO_CLK_IO;
  config.transport_config.sdio.cmd = CONFIG_EPPP_SDIO_CMD_IO;
  config.transport_config.sdio.d0 = CONFIG_EPPP_SDIO_D0_IO;
  config.transport_config.sdio.d1 = CONFIG_EPPP_SDIO_D1_IO;
  config.transport_config.sdio.d2 = CONFIG_EPPP_SDIO_D2_IO;
  config.transport_config.sdio.d3 = CONFIG_EPPP_SDIO_D3_IO;
#endif
  config.task.priority = CONFIG_EPPP_TASK_PRIORITY;
  config.task.stack_size = CONFIG_EPPP_TASK_STACK_SIZE;
  config.task.run_task = true;
  if (role == EPPP_SERVER) {
    config.ppp.our_ip4_addr.addr = esp_ip4addr_aton(CONFIG_EPPP_SERVER_IP);
    config.ppp.their_ip4_addr.addr = esp_ip4addr_aton(CONFIG_EPPP_CLIENT_IP);
  } else {
    config.ppp.our_ip4_addr.addr = esp_ip4addr_aton(CONFIG_EPPP_CLIENT_IP);
    config.ppp.their_ip4_addr.addr = esp_ip4addr_aton(CONFIG_EPPP_SERVER_IP);
  }
  config.ppp.netif_prio = 0;
  config.ppp.netif_description = NULL;
  return config;
}