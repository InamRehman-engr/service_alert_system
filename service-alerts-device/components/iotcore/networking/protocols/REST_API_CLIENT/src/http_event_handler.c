...
esp_err_t http_event_handler(esp_http_client_event_t *event_data) {
  ...
  case HTTP_EVENT_ON_DATA: {
    http_rest_recv_buffer_t *response_buffer =
        (http_rest_recv_buffer_t *)event_data->user_data;
    ...
    response_buffer->buffer =
        realloc(response_buffer->buffer,
                response_buffer->buffer_len + event_data->data_len + 1);
    ESP_LOGV(TAG, "Buffer realloced to %d bytes",
             response_buffer->buffer_len + event_data->data_len + 1);

    // Copy the new data to the buffer
    memcpy(response_buffer->buffer + response_buffer->buffer_len,
           (uint8_t *)event_data->data, event_data->data_len);
    ESP_LOGV(TAG, "Data copied to buffer");

    // Increase the buffer length
    response_buffer->buffer_len += event_data->data_len;
    ESP_LOGV(TAG, "Buffer length increased to %d bytes",
             response_buffer->buffer_len);
