...
public:
  ...
  ip_event_t get_ip_event_type() { return ip_event_type; }

private:
  static void on_event(void *arg, esp_event_base_t base, int32_t event,
                       void *data) {
  ...
  void mqtt_event(int32_t event, void *data) {
    if (mqtt_client &&
        event == mqtt_client->get_event(MqttClient::Event::CONNECT)) {
      signal.set(MQTT_Connect);
    } else if (mqtt_client &&
               event == mqtt_client->get_event(MqttClient::Event::DATA)) {
...
extern "C" void app_main(void) {
  ...
  /* The VFS terminal is just a demonstration of using an abstract file
   * descriptor which implements non-block reads, writes and selects to
   * communicate with esp-modem. This configuration uses the same UART driver as
   * the terminal created by `create_uart_dte()`, so doesn't give any practical
   * benefit besides the FD use demonstration and a placeholder to use FD
   * terminal for other devices
   */
  struct esp_modem_vfs_uart_creator uart_config =
      ESP_MODEM_VFS_DEFAULT_UART_CONFIG("/dev/uart/1");
...
#error "Unsupported device"
  ...
  if (dce->read_pin(pin_ok) == command_result::OK && !pin_ok) {
    ESP_MODEM_THROW_IF_FALSE(dce->set_pin(CONFIG_EXAMPLE_SIM_PIN) ==
                                 command_result::OK,
                             "Cannot set PIN!");
    vTaskDelay(pdMS_TO_TICKS(
        1000)); // Need to wait for some time after unlocking the SIM
  }
#endif

  if (dce->set_mode(esp_modem::modem_mode::CMUX_MODE)) {
    std::cout << "Modem has correctly entered multiplexed command/data mode"
              << std::endl;
  } else {
  ...
  if (!handler.wait_for(StatusHandler::IP_Event, 60000)) {
    ...
    if (!handler.wait_for(StatusHandler::MQTT_Connect, 60000)) {
      ESP_LOGE(TAG, "Cannot connect to %s within specified timeout... exiting",
               BROKER_URL);
      return;
    }
    ...
    if (!handler.wait_for(StatusHandler::MQTT_Data, 60000)) {
      ESP_LOGE(
          TAG,
          "Didn't receive published data within specified timeout... exiting");
      return;
    }
  ...
  for (int i = 0; i < 200; ++i) {
    if (dce->get_gnss_information_sim70xx(gps) ==
        esp_modem::command_result::OK) {
      ESP_LOGI(TAG, "gps.run  %i", gps.run);
      ESP_LOGI(TAG, "gps.fix  %i", gps.fix);
      ESP_LOGI(TAG, "gps.date.year %i gps.date.month %i gps.date.day %i",
               gps.date.year, gps.date.month, gps.date.day);
      ESP_LOGI(TAG,
               "gps.tim.hour %i gps.tim.minute %i   gps.tim.second %i   "
               "gps.tim.thousand %i",
               gps.tim.hour, gps.tim.minute, gps.tim.second, gps.tim.thousand);
      ESP_LOGI(TAG, "gps.latitude %f gps.longitude %f ", gps.latitude,
               gps.longitude);
      ESP_LOGI(TAG, "gps.altitude  %f", gps.altitude);
      ESP_LOGI(TAG, "gps.speed  %f", gps.speed);
      ESP_LOGI(TAG, "gps.cog  %f", gps.cog);
      ESP_LOGI(TAG, "gps.fix_mode  %i", gps.fix_mode);
      ESP_LOGI(TAG, "gps.dop_h %f gps.dop_p %f gps.dop_v %f ", gps.dop_h,
               gps.dop_p, gps.dop_v);
      ESP_LOGI(TAG, "gps.sats_in_view  %i", gps.sats_in_view);
      ESP_LOGI(TAG, "gps.hpa  %f gps.vpa  %f", gps.hpa, gps.vpa);
    }
