...
enum class status { IDLE, CONNECTING, SENDING, RECEIVING, FAILED, PENDING };

class Responder {
public:
  enum class ret { OK, FAIL, IN_PROGRESS, NEED_MORE_DATA, NEED_MORE_TIME };
  ...
  status pending();
  uint8_t *get_buf() { return &buffer[0]; }
  size_t get_buf_len() { return buffer_size; }

  void clear_offsets() { actual_read = 0; }

  size_t data_available() { return actual_read; }

  size_t has_data() { return total_len; }
...
std::unique_ptr<DCE> create(const esp_modem::dce_config *config,
                            std::shared_ptr<esp_modem::DTE> dte);
