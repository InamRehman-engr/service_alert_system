...
bool DCE::perform_sock() {
  ...
  int s =
      select(std::max(sock, data_ready_fd) + 1, &fdset, nullptr, nullptr, &tv);
  if (s == 0) {
...
public:
  static std::unique_ptr<DCE> create(const esp_modem::dce_config *config,
    ...
    return esp_modem::dce_factory::Factory::build_module_T<
        DCE, std::unique_ptr<DCE>>(config, std::move(dte));
  }
};
...
// ::sock_commands namespace
//
#define ESP_MODEM_DECLARE_DCE_COMMAND(name, return_type, arg_nr, ...)          \
  esp_modem::return_type DCE::name(__VA_ARGS__) {                              \
    return sock_commands::name(dte.get() ARGS(arg_nr));                        \
  }
