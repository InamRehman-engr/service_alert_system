...
static int tcp_connect(esp_transport_handle_t t, const char *host, int port,
                       int timeout_ms) {
...
static int tcp_read(esp_transport_handle_t t, char *buffer, int len,
                    int timeout_ms) {
  auto *dce = (sock_dce::DCE *)esp_transport_get_context_data(t);
...
static int tcp_write(esp_transport_handle_t t, const char *buffer, int len,
                     int timeout_ms) {
...
static int base_destroy(esp_transport_handle_t transport) { return 0; }
...
esp_transport_handle_t esp_transport_at_init(sock_dce::DCE *dce) {
  ...
  esp_transport_set_func(at, tcp_connect, tcp_read, tcp_write, base_close,
                         base_poll_read, base_poll_write, base_destroy);
  return at;
}
