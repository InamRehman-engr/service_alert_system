...
command_result net_open(CommandableIf *term) {
  ...
  std::string response;
  auto ret =
      dce_commands::generic_get_string(term, "AT+NETOPEN?\r", response, 1000);
  ...
  if (response.find("+NETOPEN: 1") != std::string::npos) {
    ...
    ret = dce_commands::generic_command(term, "AT+NETOPEN\r", "+NETOPEN: 1",
                                        "+NETOPEN: 0", 10000);
  } else {
  ...
  return dce_commands::generic_command(term, "AT+CIPRXGET=1\r", "OK", "ERROR",
                                       5000);
}

command_result net_close(CommandableIf *term) {
  ESP_LOGV(TAG, "%s", __func__);
  return dce_commands::generic_command(term, "AT+NETCLOSE\r",
                                       "+NETCLOSE:", "ERROR", 30000);
}

command_result tcp_open(CommandableIf *term, const std::string &host, int port,
                        int timeout) {
  ESP_LOGV(TAG, "%s", __func__);
  auto ret = dce_commands::generic_command(term, "AT+CIPRXGET=1\r", "OK",
                                           "ERROR", 50000);
  if (ret != command_result::OK) {
  ...
  ESP_LOGV(TAG, "%s", __func__);
  std::string ip_open =
      R"(AT+CIPOPEN=0,"TCP",")" + host + "\"," + std::to_string(port) + "\r";
  ret = dce_commands::generic_command(term, ip_open, "+CIPOPEN: 0,0", "ERROR",
                                      timeout);
  if (ret != command_result::OK) {
...
command_result tcp_close(CommandableIf *term) {
  ESP_LOGV(TAG, "%s", __func__);
  return dce_commands::generic_command(term, "AT+CIPCLOSE=0\r",
                                       "+CIPCLOSE:", "ERROR", 10000);
}

command_result tcp_send(CommandableIf *term, uint8_t *data, size_t len) {
  ESP_LOGV(TAG, "%s", __func__);
  std::string send = "AT+CIPSEND=0," + std::to_string(len) + "\r";
  auto ret = term->command(
      send,
      [&](uint8_t *data, size_t len) {
        std::string_view response((char *)data, len);
        ESP_LOGI(TAG, "CIPSEND response %.*s",
  ...
  term->on_read([&ret](uint8_t *cmd_data, size_t cmd_len) {
    ...
    ESP_LOGW(TAG, "CIPSEND response %.*s", static_cast<int>(response.size()),
             response.data());
...
command_result tcp_recv(CommandableIf *term, uint8_t *data, size_t len,
                        size_t &out_len) {
  ESP_LOGV(TAG, "%s", __func__);
  std::string out;
  ...
  size_t data_len;
  if (std::from_chars(out.data() + pattern.size(), out.data() + out.size(),
                      data_len)
  ...
  return term->command(
      "AT+CIPRXGET=2,0,100\r",
      [&](uint8_t *cmd_data, size_t cmd_len) {
...
command_result set_rx_mode(CommandableIf *term, int mode) {
  return dce_commands::generic_command(
      term, "AT+CIPRXGET=" + std::to_string(mode) + "\r", "OK", "ERROR", 5000);
}
...
bool Responder::start_connecting(std::string host, int port) {
  send_cmd(R"(AT+CIPOPEN=0,"TCP",")" + host + "\"," + std::to_string(port) +
           "\r");
  return true;
}
...
Responder::ret Responder::recv(uint8_t *data, size_t len) {
  ...
  if (data_to_recv == 0) {
    ...
    auto head_pos =
        std::search(recv_data, recv_data + len, head.begin(), head.end());
    ...
    if (std::from_chars(head_pos + head.size(), next_comma, actual_len).ec ==
    ...
    total_len = 0;
    if (std::from_chars(next_comma + 1, next_nl - 1, total_len).ec ==
        std::errc::invalid_argument) {
      ESP_LOGE(TAG, "cannot convert");
      return ret::FAIL;
    }
    ...
    recv_data = next_nl + 1;
    auto first_data_len =
        len - (recv_data - (char *)data) /* minus size of the command marker */;
  ...
  if (last_pos != nullptr && (char *)data + len - last_pos - 2 > MIN_MESSAGE) {
    ...
    std::string_view response((char *)last_pos + 2 /* OK */,
                              (char *)data + len - last_pos - 2);
    check_async_replies(status::RECEIVING, response);
  }
  data_to_recv = 0;
  if (read_again) {
...
Responder::ret Responder::check_async_replies(status state,
                                              std::string_view &response) {
  ESP_LOGD(TAG, "response %.*s", static_cast<int>(response.size()),
           response.data());
...
Responder::ret Responder::process_data(status state, uint8_t *data,
                                       size_t len) {
  if (state == status::SENDING) {
...
status Responder::pending() { return status::PENDING; }
