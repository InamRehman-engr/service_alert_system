...
struct MqttClientHandle {
  ...
  ~MqttClientHandle() { esp_mqtt_client_destroy(client); }
...
void MqttClient::connect() { esp_mqtt_client_start(h->client); }
...
int MqttClient::publish(const std::string &topic, const std::string &data,
                        int qos) {
  return esp_mqtt_client_publish(h->client, topic.c_str(), data.c_str(), 0, qos,
                                 0);
}
...
void MqttClient::register_handler(int32_t event_id,
                                  esp_event_handler_t event_handler,
                                  void *arg) {
  ESP_ERROR_CHECK(esp_mqtt_client_register_event(h->client, MQTT_EVENT_ANY,
                                                 event_handler, arg));
}
