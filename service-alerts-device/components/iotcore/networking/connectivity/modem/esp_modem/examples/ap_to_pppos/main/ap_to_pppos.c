...
void start_network(void) {
  ...
  while ((bits & CONNECT_BIT) == 0) {
    if (!modem_check_sync()) {
      ESP_LOGE(TAG, "Modem does not respond, maybe in DATA mode? ...exiting "
                    "network mode");
      modem_stop_network();
      if (!modem_check_sync()) {
    ...
    if (!modem_start_network()) {
      ESP_LOGE(TAG,
               "Modem could not enter network mode ...will retry after 10s");
...
void app_main(void) {
  ...
  while (true) {
    EventBits_t bits = xEventGroupWaitBits(event_group, DISCONNECT_BIT, pdTRUE,
