 ...
 */
struct CommandArgs {
  CommandArgs(arg_type t, const char *shopts, const char *lopts,
              const char *data, const char *glos)
      : type(t), shortopts(shopts), longopts(lopts), datatype(data),
        glossary(glos) {}
  CommandArgs(arg_type t, const char *shopts, const char *lopts,
              const char *glos)
      : type(t), shortopts(shopts), longopts(lopts), datatype(nullptr),
        glossary(glos) {}
...
/**
 * @brief This class simplifies console command definition in more object wise
 * fashion
 */
...
public:
   ...
   */
  template <typename T>
  explicit ConsoleCommand(const char *command, const char *help,
                          const T *arg_struct, size_t srg_struct_size,
  ...
  /**
   * @brief Another method of Console command definitions using vector arg
   * struct
   */
  explicit ConsoleCommand(const char *command, const char *help,
...
private:
  ...
  void RegisterCommand(const char *command, const char *help,
                       const std::vector<CommandArgs> &args);
  template <typename T>
  static constexpr size_t index_arg(CommandArgs T::*member) {
