...
bool Tls::init(is_server server, do_verify verify) {
  ...
  int ret = mbedtls_ssl_config_defaults(
      &conf_,
      server == is_server{true} ? MBEDTLS_SSL_IS_SERVER : MBEDTLS_SSL_IS_CLIENT,
  ...
  mbedtls_ssl_conf_authmode(&conf_, verify == do_verify{true}
...
int Tls::mbedtls_pk_parse_key(mbedtls_pk_context *ctx, const unsigned char *key,
                              size_t keylen, const unsigned char *pwd,
                              size_t pwdlen) {

  return ::mbedtls_pk_parse_key(ctx, key, keylen, pwd, pwdlen, nullptr,
                                nullptr);
}
...
void Tls::reset_session() { session_.reset(nullptr); }
bool Tls::is_session_loaded() { return session_ != nullptr; }
