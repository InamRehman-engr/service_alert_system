...
#define ESP_MODEM_DECLARE_DCE_COMMAND(name, return_type, arg_nr, ...)          \
  return_type Shiny::DCE::name(__VA_ARGS__) {                                  \
    return esp_modem::dce_commands::name(this ARGS(arg_nr));                   \
  }
...
/**
 * @brief Definition of the command API, which makes the Shiny::DCE
 * "command-able class"
 ...
 */
command_result Shiny::DCE::command(const std::string &cmd, got_line_cb got_line,
