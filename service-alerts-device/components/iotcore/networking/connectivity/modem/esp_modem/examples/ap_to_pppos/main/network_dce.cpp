...
public:
  ...
  template <typename Module, typename... Args>
  static std::shared_ptr<Module> create_module(const config *cfg,
                                               Args &&...args) {
...
/**
 * @brief This is an example of implementing minimal network module
 * functionality
 *
 * This does only include those AT commands, that are needed for setting the
 * network up and also initialization part (set pin, ...)
 */
...
public:
  ...
  bool check_signal() {
    int rssi, ber;
    if (dce_commands::get_signal_quality(dte.get(), rssi, ber) !=
        command_result::OK) {
      return false;
    }
  ...
  [[maybe_unused]] bool init_sim(const std::string &pin) {
    ...
    static_cast<void>(
        set_command_mode()); // ignore the potential failure, as we might be in
                             // command mode after startup
    bool is_pin_ok;
...
extern "C" esp_err_t modem_init_network(esp_netif_t *netif) {
  ...
  auto dev =
      NetDCE_Factory::create_module<NetModule>(&dce_config, uart_dte, netif);
