...
class LocalFactory : public Factory {
  using DCE_gnss_ret =
      std::unique_ptr<DCE_gnss>; // this custom Factory manufactures only
                                 // unique_ptr<DCE>'s
public:
  static DCE_gnss_ret create(const dce_config *config, std::shared_ptr<DTE> dte,
                             esp_netif_t *netif) {
...
/**
 * @brief Helper create method which employs the DCE factory for creating DCE
 * objects templated by a custom module
 * @return unique pointer of the resultant DCE
 */
std::unique_ptr<DCE_gnss>
create_SIM7070_GNSS_dce(const esp_modem::dce_config *config,
...
get_gnss_information_sim70xx_lib(esp_modem::CommandableIf *t,
                                 esp_modem_gps_t &gps) {

  ESP_LOGV(TAG, "%s", __func__);
  std::string str_out;
  auto ret =
      esp_modem::dce_commands::generic_get_string(t, "AT+CGNSINF\r", str_out);
  ...
  // sats_in_view
  {
    ...
    if (sats_in_view.length() > 1) {
      if (std::from_chars(out.data(), out.data() + pos, gps.sats_in_view).ec ==
          std::errc::invalid_argument) {
        return esp_modem::command_result::FAIL;
      }
    } else {
...
esp_modem::command_result
SIM7070_gnss::get_gnss_information_sim70xx(esp_modem_gps_t &gps) {
  return get_gnss_information_sim70xx_lib(dte.get(), gps);
}

esp_modem::command_result
DCE_gnss::get_gnss_information_sim70xx(esp_modem_gps_t &gps) {
