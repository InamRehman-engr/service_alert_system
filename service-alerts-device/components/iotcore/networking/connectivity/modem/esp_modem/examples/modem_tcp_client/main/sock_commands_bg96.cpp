...
command_result net_open(CommandableIf *t) {
  ...
  if (out.find("+QISTATE: 0") != std::string::npos) {
    ...
    return dce_commands::generic_command(t, "AT+QIACT=1\r", "OK", "ERROR",
                                         150000);
  }
  return command_result::FAIL;
}
...
command_result net_close(CommandableIf *t) {
  ...
  return dce_commands::generic_command(t, "AT+QIDEACT=1\r", "OK", "ERROR",
                                       40000);
}

command_result tcp_open(CommandableIf *t, const std::string &host, int port,
                        int timeout) {
  ESP_LOGV(TAG, "%s", __func__);
  std::string ip_open =
      R"(AT+QIOPEN=1,0,"TCP",")" + host + "\"," + std::to_string(port) + "\r";
  auto ret = dce_commands::generic_command(t, ip_open, "+QIOPEN: 0,0", "ERROR",
                                           timeout);
  if (ret != command_result::OK) {
...
command_result tcp_close(CommandableIf *t) {
  ESP_LOGV(TAG, "%s", __func__);
  return dce_commands::generic_command(t, "AT+QICLOSE=0\r", "OK", "ERROR",
                                       10000);
}
...
command_result get_ip(CommandableIf *t, std::string &ip) {
  ...
  while (pos != std::string::npos) {
    // Looking for: +QIACT:
    // <contextID>,<context_state>,<context_type>,<IP_address>
    if (property++ ==
        3) { // ip is after 3rd comma (as a 4rd property of QIACT string)
      ip = out.substr(++pos);
...
bool Responder::start_connecting(std::string host, int port) {
  send_cmd(R"(AT+QIOPEN=1,0,"TCP",")" + host + "\"," + std::to_string(port) +
           "\r");
  return true;
}
...
Responder::ret Responder::recv(uint8_t *data, size_t len) {
  ...
  if (data_to_recv == 0) {
    ...
    if (std::from_chars(head_pos + head.size(), next_nl, actual_len).ec ==
  ...
  if (send_stat == 3) {
    ...
    if (response.find(head) != std::string::npos) {
      ...
      while ((pos = response.find(',')) != std::string::npos) {
        ...
        // extract value
        size_t value;
        if (std::from_chars(response.data(), next_comma, value).ec ==
            std::errc::invalid_argument) {
          ESP_LOGE(TAG, "cannot convert");
          return ret::FAIL;
        }
      ...
      if (std::from_chars(response.data(), response.data() + pos, unack).ec ==
          std::errc::invalid_argument) {
        return ret::FAIL;
      }

      if (ack < total) {
        ESP_LOGD(TAG, "all sending data are not ack (missing %d bytes acked)",
                 (total - ack));
        if (total - ack > 64) {
