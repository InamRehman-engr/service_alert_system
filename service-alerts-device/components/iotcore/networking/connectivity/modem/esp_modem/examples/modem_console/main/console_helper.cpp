...
ConsoleCommand::ConsoleCommand(const char *command, const char *help,
                               const std::vector<CommandArgs> &args,
                               std::function<bool(ConsoleCommand *)> f)
    : func(std::move(f)) {
...
void ConsoleCommand::RegisterCommand(const char *command, const char *help,
                                     const std::vector<CommandArgs> &args) {
  ...
  for (auto &it : args) {
    ...
    case STR0:
      common_arg.str =
          arg_str0(it.shortopts, it.longopts, it.datatype, it.glossary);
      break;
    case STR1:
      common_arg.str =
          arg_str1(it.shortopts, it.longopts, it.datatype, it.glossary);
      break;
    case INT0:
      common_arg.intx =
          arg_int0(it.shortopts, it.longopts, it.datatype, it.glossary);
      break;
    case INT1:
      common_arg.intx =
          arg_int1(it.shortopts, it.longopts, it.datatype, it.glossary);
      break;
    case LIT0:
  ...
  const esp_console_cmd_t command_def = {.command = command,
                                         .help = help,
                                         .hint = nullptr,
...
/**
 * @brief This class holds definitions of static functions, numbered from 0
 * index, that call indexed methods of ConsoleCommand (used to bridge from
 * static esp-console to object oriented ConsoleCommand class)
 */
...
/**
 * @brief ConsoleCommand list of static callbacks used for getting object
 * context to plain esp-console context
 */
