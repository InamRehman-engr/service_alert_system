...
static void cmd_ping_on_ping_success(esp_ping_handle_t hdl, void *args) {
  ...
  esp_ping_get_profile(hdl, ESP_PING_PROF_TIMEGAP, &elapsed_time,
                       sizeof(elapsed_time));
  ESP_LOGI(
      TAG, "%" PRIu32 " bytes from %s icmp_seq=%d ttl=%d time=%" PRIu32 " ms\n",
...
static void cmd_ping_on_ping_end(esp_ping_handle_t hdl, void *args) {
  ...
  ESP_LOGI(TAG,
           "%" PRIu32 " packets transmitted, %" PRIu32 " received, %" PRIu32
...
void modem_console_register_ping(void) {
  ping_args.timeout = arg_dbl0("W", "timeout", "<t>",
                               "Time to wait for a response, in seconds");
  ping_args.count =
      arg_int0("c", "count", "<n>", "Stop after sending count packets");
