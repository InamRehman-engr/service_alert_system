...
static const int USB_DISCONNECTED_BIT =
    BIT3; // Used only with USB DTE but we define it unconditionally, to avoid
...
#define CHECK_USB_DISCONNECTION(event_group)                                   \
  if ((xEventGroupGetBits(event_group) & USB_DISCONNECTED_BIT) ==              \
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base,
  ...
  case MQTT_EVENT_CONNECTED:
    ...
    msg_id =
        esp_mqtt_client_subscribe(client, CONFIG_EXAMPLE_MQTT_TEST_TOPIC, 0);
  ...
  case MQTT_EVENT_SUBSCRIBED:
    ...
    msg_id = esp_mqtt_client_publish(client, CONFIG_EXAMPLE_MQTT_TEST_TOPIC,
                                     CONFIG_EXAMPLE_MQTT_TEST_DATA, 0, 0, 0);
...
void app_main(void) {
  ...
  esp_modem_dce_t *dce = esp_modem_new_dev(ESP_MODEM_DCE_CUSTOM, &dte_config,
                                           &dce_config, esp_netif);
#else
  ...
  if (dte_config.uart_config.flow_control == ESP_MODEM_FLOW_CONTROL_HW) {
    esp_err_t err =
        esp_modem_set_flow_control(dce, 2, 2); // 2/2 means HW Flow Control.
    if (err != ESP_OK) {
...
#error USB modem not selected
    ...
    esp_modem_dce_t *dce = esp_modem_new_dev_usb(usb_dev_type, &dte_usb_config,
                                                 &dce_config, esp_netif);
    assert(dce);
...
#error Invalid serial connection to modem.
  ...
  xEventGroupClearBits(event_group,
                       CONNECT_BIT | GOT_DATA_BIT | USB_DISCONNECTED_BIT);
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "esp_modem_get_signal_quality failed with %d %s", err,
             esp_err_to_name(err));
    return;
  }
...
#ifdef CONFIG_EXAMPLE_MODEM_DEVICE_CUSTOM
  {
    ...
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "esp_modem_get_time failed with %d %s", err,
               esp_err_to_name(err));
      return;
    }
  ...
  err = esp_modem_send_sms(dce, CONFIG_EXAMPLE_SEND_MSG_PEER_PHONE_NUMBER,
                           "Text message from esp-modem");
  if (err != ESP_OK) {
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "esp_modem_set_mode(ESP_MODEM_MODE_DATA) failed with %d",
             err);
    return;
  }
  /* Wait for IP address */
  ESP_LOGI(TAG, "Waiting for IP address");
  xEventGroupWaitBits(event_group, CONNECT_BIT | USB_DISCONNECTED_BIT, pdFALSE,
  ...
  xEventGroupWaitBits(event_group, GOT_DATA_BIT | USB_DISCONNECTED_BIT, pdFALSE,
                      pdFALSE, portMAX_DELAY);
  CHECK_USB_DISCONNECTION(event_group);
  ...
  ESP_LOGI(
      TAG,
      "USB demo finished. Disconnect and connect the modem to run it again");
  xEventGroupWaitBits(event_group, USB_DISCONNECTED_BIT, pdFALSE, pdFALSE,
                      portMAX_DELAY);
