...
/**
 * @brief Definition of a custom modem which inherits from the GenericModule,
 * uses all its methods and could override any of them. Here, for demonstration
 * purposes only, we redefine just `get_module_name()`
 */
...
public:
  ...
  command_result command(const std::string &cmd, got_line_cb got_line,
                         uint32_t time_ms, const char separator) override;
...
public:
  static std::unique_ptr<DCE> create(const esp_modem::dce_config *config,
                                     ...
                                     esp_netif_t *netif) {
    return build_generic_DCE<MyShinyModem, DCE, std::unique_ptr<DCE>>(
 ...
 */
std::unique_ptr<Shiny::DCE>
create_shiny_dce(const esp_modem::dce_config *config,
                 std::shared_ptr<esp_modem::DTE> dte, esp_netif_t *netif);
