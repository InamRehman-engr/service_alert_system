...
TEST_CASE("Connect test", "[esp_modem]") {
  EventBits_t b = xEventGroupWaitBits(event_group, 1, pdTRUE, pdFALSE,
                                      pdMS_TO_TICKS(15000));
  CHECK(b == 1);
}
...
TEST_CASE("Disconnection test", "[esp_modem]") {
  modem_stop_network();
  EventBits_t b = xEventGroupWaitBits(event_group, 2, pdTRUE, pdFALSE,
                                      pdMS_TO_TICKS(15000));
  CHECK(b == 2);
}
...
static void handle(int nr) { ESP_LOGE(TAG, "Signal handler %d", nr); }

_sig_func_ptr signal(int nr, _sig_func_ptr) { return handle; }
}
