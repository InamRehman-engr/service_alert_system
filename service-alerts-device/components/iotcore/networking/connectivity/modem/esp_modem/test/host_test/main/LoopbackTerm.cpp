...
void LoopbackTerm::start() { status = status_t::STARTED; }

void LoopbackTerm::stop() { status = status_t::STOPPED; }

int LoopbackTerm::write(uint8_t *data, size_t len) {
  if (inject_by) { // injection test: ignore what we write, but respond with
                   // injected data
    signal.clear(1);
  ...
  if (len > 2 &&
      (data[len - 1] == '\r' || data[len - 1] == '+')) { // Simple AT responder
...
int LoopbackTerm::read(uint8_t *data, size_t len) {
  ...
  if (read_len) {
    ...
    loopback_data.erase(loopback_data.begin(),
                        loopback_data.begin() + read_len);
    data_len -= read_len;
  }
  return read_len;
}

LoopbackTerm::LoopbackTerm(bool is_bg96)
    : loopback_data(), data_len(0), pin_ok(false), is_bg96(is_bg96),
      inject_by(0) {
  init_signal();
}

LoopbackTerm::LoopbackTerm()
    : loopback_data(), data_len(0), pin_ok(false), is_bg96(false),
      inject_by(0) {
  init_signal();
}

int LoopbackTerm::inject(uint8_t *data, size_t len, size_t injected_by,
                         size_t delay_before, size_t delay_after) {
  if (data == nullptr) {
    inject_by = 0;
    return 0;
  }
...
void LoopbackTerm::init_signal() {
  ...
  // we clear the signal upon an async operation, so the destructor needs to
