...
TEST_CASE("DTE command races", "[esp_modem]") {
  ...
  CHECK(dce != nullptr);
  uint8_t resp[] = {'O', 'K', '\n'};
  // run many commands in succession with the timeout set exactly to the
  // timespan of injected reply (checks for potential exception, data races,
  // recycled local variables, etc.)
  for (int i = 0; i < 1000; ++i) {
    loopback->inject(&resp[0], sizeof(resp), sizeof(resp),
                     /* 1ms before injecting reply */ 1, 0);
    auto ret = dce->command(
        "AT\n", [&](uint8_t *data, size_t len) { return command_result::OK; },
        1);
    // this command should either timeout or finish successfully
    CHECK((ret == command_result::TIMEOUT || ret == command_result::OK));
  }

  // Now we test the same, but with some garbage after the expected data and
  // inject the reply in chunks by 3 bytes
  uint8_t resp2[] = {'O', 'K', '\n', '1', '2', '\n'};
  for (int i = 0; i < 1000; ++i) {
    loopback->inject(&resp2[0], sizeof(resp2), 3,
                     /* 1ms before injecting reply */ 0, 0);
    auto ret = dce->command(
        "check\n",
        [&](uint8_t *data, size_t len) {
          if (len > 0 &&
              data[0] == 'O') { // expected reply only when it starts with '0'
...
TEST_CASE("Test polymorphic delete for custom device/dte", "[esp_modem]") {
  ...
  // Create custom device and DTE manually to check for a potential undefined
  // behaviour
...
TEST_CASE("Test CMUX protocol by injecting payloads", "[esp_modem]") {
  ...
  const auto test_command = "Test\n";
  // 1 byte payload size
  uint8_t test_payload[] = {0xf9, 0x09, 0xff, 0x0b, 0x54, 0x65,
                            0x73, 0x74, 0x0a, 0xbb, 0xf9};
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    loopback->inject(&long_payload[0], sizeof(long_payload),
                     i == 0 ? sizeof(long_payload) : i);
    auto ret = dce->command(
        "ignore",
        [&](uint8_t *data, size_t len) {
          CHECK(data[0] == 0x7e);
...
TEST_CASE("Command and Data mode transitions", "[esp_modem][transitions]") {
  ...
  loopback->inject(&resp[0], sizeof(resp), sizeof(resp),
                   /* 10ms before injecting reply */ 100, 0);
  loopback->write(nullptr, 0); /* this triggers sending the injected response */
  CHECK(dce->set_mode(esp_modem::modem_mode::COMMAND_MODE) == true);
  loopback->inject(nullptr, 0, 0, 0,
                   0); /* reset injection, use synchronous replies now */
  // CMD -> CMD (Fail)
  CHECK(dce->set_mode(esp_modem::modem_mode::COMMAND_MODE) == false);
...
TEST_CASE("CMUX manual mode transitions", "[esp_modem][transitions]") {
  ...
  // Cannot test CMUX_MANUAL_DATA -> CMUX_MANUAL_COMMAND with our mocked
  // terminal for now
  CHECK(dce->set_mode(esp_modem::modem_mode::CMUX_MANUAL_EXIT) == true);
  ...
  CHECK(dce->set_mode(esp_modem::modem_mode::CMUX_MANUAL_SWAP) ==
        false); // cannot go directly to SWAP
  CHECK(dce->set_mode(esp_modem::modem_mode::UNDEF) == true);
  CHECK(dce->set_mode(esp_modem::modem_mode::CMUX_MANUAL_SWAP) ==
        true); // can go via UNDEF
  CHECK(dce->set_mode(esp_modem::modem_mode::CMUX_MANUAL_EXIT) ==
        false); // EXIT is allowed, but CMUX terms don't exist
  CHECK(dce->set_mode(esp_modem::modem_mode::UNDEF) == true);
  CHECK(dce->set_mode(esp_modem::modem_mode::CMUX_MANUAL_MODE) ==
        true); // Enter CMUX (via UNDEF)
  CHECK(dce->set_mode(esp_modem::modem_mode::CMUX_MANUAL_DATA) ==
        true); // Go directly to DATA mode
  CHECK(dce->set_mode(esp_modem::modem_mode::CMUX_MANUAL_EXIT) ==
        true); // Exit CMUX
  CHECK(dce->set_mode(esp_modem::modem_mode::UNDEF) ==
        true); // Succeeds from any state
}
