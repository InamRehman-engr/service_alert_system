...
struct esp_modem_uart_term_config {
  ...
  int event_queue_size; /*!< UART Event Queue Size, set to 0 if no event queue
                           needed */
};
...
struct esp_modem_vfs_term_config {
  ...
  void (*deleter)(
      int,
      struct esp_modem_vfs_resource *); /*!< Custom close function for the fd */
  struct esp_modem_vfs_resource
      *resource; /*!< Resource attached to the VFS (need for clenaup) */
};
...
struct esp_modem_dte_config {
  ...
  union {
    struct esp_modem_uart_term_config
        uart_config; /*!< Configuration for UART Terminal */
    struct esp_modem_vfs_term_config
        vfs_config;         /*!< Configuration for VFS Terminal */
...
#define ESP_MODEM_DTE_DEFAULT_CONFIG()                                         \
  {                                                                            \
    .dte_buffer_size = 512, .task_stack_size = 4096, .task_priority = 5,       \
    .uart_config = {                                                           \
        .port_num = UART_NUM_1,                                                \
