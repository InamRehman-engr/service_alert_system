...
#endif
#define ESP_MODEM_THROW_IF_FALSE(...)                                          \
  esp_modem::throw_if_false(__FILENAME__, __LINE__, __VA_ARGS__)
#define ESP_MODEM_THROW_IF_ERROR(...)                                          \
  esp_modem::throw_if_error(__FILENAME__, __LINE__, __VA_ARGS__)
...
public:
  explicit esp_err_exception(std::string msg)
      : esp_err(ESP_FAIL), message(std::move(msg)) {}
  explicit esp_err_exception(std::string msg, esp_err_t err)
      : esp_err(err), message(std::move(msg)) {}
...
#else
#define ESP_MODEM_THROW(exception)                                             \
  do {                                                                         \
    exception;                                                                 \
    abort();                                                                   \
  } while (0)
...
static inline std::string make_message(const std::string &filename, int line,
                                       const std::string &message = "ERROR") {
  std::string text = filename + ":" + std::to_string(line) + " " + message;
  return text;
}

static inline void throw_if_false(const std::string &filename, int line,
...
static inline void throw_if_error(const std::string &filename, int line,
                                  esp_err_t err, const std::string &message) {
  if (err != ESP_OK) {
    ESP_MODEM_THROW(
        esp_err_exception(make_message(filename, line, message), err));
  }
}
