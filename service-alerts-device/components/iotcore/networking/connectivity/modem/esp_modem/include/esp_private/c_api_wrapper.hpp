 ...
 *
 * This file is located in include/esp_private because it is not intended for
 * users, but rather for esp_modem C extension developers.
 *
 * The C extension API must provide a 'factory function' that returns
 * initialized pointer to esp_modem_dce_wrap. Helper functions provided below,
 * can be used for conversion between C++ enum classes and standard C enums.
 */
...
struct esp_modem_dce_wrap { // need to mimic the polymorphic dispatch as CPP
                            // uses templated dispatch
...
inline dce_factory::ModemType
convert_modem_enum(esp_modem_dce_device_t module) {
  switch (module) {
  ...
  case ESP_MODEM_DCE_A76XX:
    return esp_modem::dce_factory::ModemType::A76XX;
  default:
...
inline esp_modem_terminal_error_t
convert_terminal_error_enum(terminal_error err) {
  switch (err) {
