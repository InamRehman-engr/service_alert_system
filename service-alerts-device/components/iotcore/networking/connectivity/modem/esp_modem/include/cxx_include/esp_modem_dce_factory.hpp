...
/**
 * @brief Helper class for creating a user define pointer in a specific way,
 * either as a plain pointer, shared_ptr or unique_ptr
 */
class FactoryHelper {
public:
  ...
  static auto make(Args &&...args) ->
      typename std::enable_if<std::is_same<T_Ptr, T *>::value, T *>::type {
    return new T(std::forward<Args>(args)...);
  }

  template <typename T, typename T_Ptr, typename... Args>
  static auto make(Args &&...args) ->
      typename std::enable_if<std::is_same<T_Ptr, std::shared_ptr<T>>::value,
  ...
  static auto make(Args &&...args) ->
      typename std::enable_if<std::is_same<T_Ptr, std::unique_ptr<T>>::value,
...
/**
 * @brief Creator class for building a DCE_T<Module> in a specific way, either
 * from a Module object or by default from the DTE and netif
 *
 * @throws
 *      - esp_modem::esp_err_exception on invalid arguments
 *      - std::bad_alloc if failed to allocate
 */
template <typename T_Module> class Creator {
  static_assert(std::is_base_of<ModuleIf, T_Module>::value,
                "Builder must be used only for Module classes");

public:
  Creator(std::shared_ptr<DTE> dte, esp_netif_t *esp_netif)
      : dte(std::move(dte)), device(nullptr), netif(esp_netif) {
    ESP_MODEM_THROW_IF_FALSE(netif != nullptr, "Null netif");
  }

  Creator(std::shared_ptr<DTE> dte, esp_netif_t *esp_netif,
          std::shared_ptr<T_Module> dev)
      : dte(std::move(dte)), device(std::move(dev)), netif(esp_netif) {
    ESP_MODEM_THROW_IF_FALSE(netif != nullptr, "Null netif");
  }

  explicit Creator(std::shared_ptr<DTE> dte)
      : dte(std::move(dte)), device(nullptr), netif(nullptr) {}

  ~Creator() {
    if (device != nullptr) {
      ESP_LOGE("dce_factory::~Creator",
               "module was captured or created but never used");
    }
  }
  ...
  auto create(const esp_modem_dce_config *config) -> T_Ptr {
    ...
    return FactoryHelper::make<T_Dce, T_Ptr>(std::move(dte), std::move(device),
                                             netif);
  }
...
enum class ModemType {
  ...
  SIM7600, /*!< Derived from the GenericModule, specifics applied to SIM7600
              model */
  SIM7070, /*!< Derived from the GenericModule, specifics applied to SIM7070
              model */
  SIM7000, /*!< Derived from the GenericModule, specifics applied to SIM7000
              model */
  BG96,  /*!< Derived from the GenericModule, specifics applied to BG69 model */
  A76XX, /*!< Derived from the GenericModule, specifics applied to A76XX model
          */
  SIM800, /*!< Derived from the GenericModule with specifics applied to SIM800
             model */
};

/**
 * @brief Factory class for creating virtual DCE objects based on the
 * configuration of the supplied module. This could also be used to create a
 * custom module or a DCE_T<module>, provided user app derives from this
 * factory.
 */
class Factory {
public:
   ...
   * @tparam Args Arguments to the builder, i.e. constructor of esp_modem::DCE_T
   * class
  ...
  static std::unique_ptr<DCE> build_unique(const config *cfg, Args &&...args) {
    return build_generic_DCE<T_Module, DCE, std::unique_ptr<DCE>>(
  ...
  template <typename T_Module, typename... Args>
  static std::shared_ptr<T_Module> build_shared_module(const config *cfg,
                                                       Args &&...args) {
    return build_module_T<T_Module>(cfg, std::forward<Args>(args)...);
  }

  template <typename... Args>
  std::shared_ptr<GenericModule> build_shared_module(const config *cfg,
                                                     Args &&...args) {
    switch (m) {
    ...
    case ModemType::A76XX:
      return build_shared_module<A76XX>(cfg, std::forward<Args>(args)...);
    case ModemType::GenericModule:
      return build_shared_module<GenericModule>(cfg,
  ...
  /**
   * @brief Create a default unique_ptr DCE generically, with the chosen module
   * derived from the GenericModule
   * @tparam Args Arguments to the builder, i.e. constructor of esp_modem::DCE_T
   * class
   * @param cfg DCE configuration structure ::esp_modem_dte_config
   * @param args typically a DTE object and a netif handle for PPP network
  ...
  std::unique_ptr<DCE> build_unique(const config *cfg, Args &&...args) {
    ...
    case ModemType::A76XX:
      return build_unique<A76XX>(cfg, std::forward<Args>(args)...);
    case ModemType::GenericModule:
  ...
  template <typename... Args> DCE *build(const config *cfg, Args &&...args) {
    switch (m) {
    case ModemType::SIM800:
    ...
    case ModemType::A76XX:
      return build<A76XX>(cfg, std::forward<Args>(args)...);
    case ModemType::GenericModule:
...
protected:
  template <typename T_Module, typename Ptr = std::shared_ptr<T_Module>,
            typename... Args>
  ...
  template <typename T_Module, typename T_Dce = DCE_T<T_Module>,
            typename T_DcePtr = T_Dce *, typename... Args>
