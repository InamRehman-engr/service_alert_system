...
/**
 * @brief This is a basic building block for custom modules as well as for the
 * supported modules in the esp-modem component It derives from the ModuleIf.
 */
class GenericModule : public ModuleIf {
public:
  /**
   * @brief We can construct a generic device with an existent DTE and it's
   * configuration The configuration could be either the dce-config struct or
   * just a pdp context
   */
  ...
  explicit GenericModule(std::shared_ptr<DTE> dte,
                         const esp_modem_dce_config *config);

  /**
   * @brief This is a mandatory method for ModuleIf class, which sets up the
   * device to be able to connect to the network. This typically consists of
   * setting basic communication parameters and setting the PDP (defining
   * logical access point to cellular network)
   */
  ...
  /**
   * @brief Simplified version of operator name (without the ACT, which is
   * included in the command library)
   */
...
protected:
  std::shared_ptr<DTE> dte; /*!< Generic device needs the DTE as a channel talk
                               to the module using AT commands */
  std::unique_ptr<PdpContext>
      pdp; /*!< It also needs a PDP data, const information used for setting up
              cellular network */
};

// Definitions of other supported modules with some specific commands
// overwritten
...
public:
  ...
  command_result get_gnss_power_mode(int &mode) override;
  command_result set_gnss_power_mode(int mode) override;
  command_result set_network_bands(const std::string &mode, const int *bands,
                                   int size) override;
  command_result get_gps_location(esp_modem::gps &gps) override;
};
...
/**
 * @brief Specific definition of the A76XX module
 */
class A76XX : public GenericModule {
  using GenericModule::GenericModule;
