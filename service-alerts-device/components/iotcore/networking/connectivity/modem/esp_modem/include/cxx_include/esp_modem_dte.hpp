...
struct DTE_Command {
  DTE_Command(const std::string &cmd)
      : data((uint8_t *)cmd.c_str()), len(cmd.length()) {}
...
public:
   ...
   */
  command_result command(const std::string &command, got_line_cb got_line,
   ...
   */
  command_result command(const std::string &command, got_line_cb got_line,
                         uint32_t time_ms, char separator) override;
...
protected:
  ...
  friend class Scoped<DTE>; /*!< Declaring "Scoped<DTE> lock(dte)" locks this
                               instance */
private:
  void
  handle_error(terminal_error err); /*!< Performs internal error handling */
  ...
  std::shared_ptr<Terminal> primary_term; /*!< Reference to the primary terminal
                                             (mostly for sending commands) */
  std::shared_ptr<Terminal>
      secondary_term; /*!< Secondary terminal for this DTE */
  modem_mode mode;    /*!< DTE operation mode */
  std::function<bool(uint8_t *data, size_t len)>
      on_data; /*!< on data callback for current terminal */
  std::function<void(terminal_error err)>
      user_error_cb; /*!< user callback on error event from attached terminals
                      */

#ifdef CONFIG_ESP_MODEM_USE_INFLATABLE_BUFFER_IF_NEEDED
  /**
   * @brief Implements an extra buffer that is used to capture partial reads
   * from underlying terminals when we run out of the standard buffer
   */
  struct extra_buffer {
    extra_buffer() : buffer(nullptr) {}
    ~extra_buffer() { delete buffer; }
    ...
    [[nodiscard]] uint8_t *begin() const { return &buffer->at(0); }
    [[nodiscard]] uint8_t *current() const { return &buffer->at(0) + consumed; }
  } inflatable;
#endif // CONFIG_ESP_MODEM_USE_INFLATABLE_BUFFER_IF_NEEDED

  /**
   * @brief Set internal command callbacks to the underlying terminal.
   * Here we capture command replies to be processed by supplied command
   * callbacks in  struct command_cb.
   */
  void set_command_callbacks();

  /**
   * @brief This abstracts command callback processing and implements its
   * locking, signaling of completion and timeouts.
   */
  struct command_cb {
    static const size_t GOT_LINE =
        SignalGroup::bit0; /*!< Bit indicating response available */
    ...
    SignalGroup
        signal; /*!< Event group used to signal request-response operations */
    bool process_line(uint8_t *data, size_t consumed,
                      size_t len); /*!< Lets the processing callback handle one
    ...
    void set(got_line_cb l,
             char s = '\n') /*!< Sets the command callback atomically */
    {
      Scoped<Lock> lock(line_lock);
      if (l) {
        // if we set the line callback, we have to reset the signal and the
        // result
        signal.clear(GOT_LINE);
        result = command_result::TIMEOUT;
      } else {
        // if we clear the line callback, we check consistency (since we've
        // locked the line processing)
        if (signal.is_any(command_cb::GOT_LINE) &&
            result == command_result::TIMEOUT) {
    ...
    void give_up() /*!< Reports other than timeout error when processing replies
                      (out of buffer) */
    {
