 ...
 *
 * This class inherits from Terminal class, as it is a Terminal, but is also
 * composed of another Terminal, which is used to communicate with the modem,
 * i.e. the original Terminal which has been multiplexed.
 *
...
/**
 * @brief CMux class which consumes the original terminal and creates multiple
 * virtual terminals from it. This class itself is not usable as a DTE terminal,
 * only via its instances defined in `CMuxInstance`
 */
class CMux {
public:
  explicit CMux(std::shared_ptr<Terminal> t, unique_buffer &&b)
      : term(std::move(t)), payload_start(nullptr), total_payload_size(0),
        buffer(std::move(b)) {}
  ~CMux() = default;
   ...
   *
   * This restarts the CMUX state machine, which could have been in a wrong
   * state due to communication issue on a lower layer.
   *
   * @return true on success
   */
  bool recover();
...
private:
  ...
  static uint8_t
  fcs_crc(const uint8_t frame[6]); /*!< Utility to calculate FCS CRC */
  bool
  data_available(uint8_t *data,
                 size_t len);     /*!< Called when valid data available (returns
                                     false on unexpected data format) */
  void send_sabm(size_t i);       /*!< Sending initial SABM */
  void send_disconnect(size_t i); /*!< Sending closing request for each virtual
                                     or control terminal */
  bool on_cmux_data(uint8_t *data,
                    size_t len); /*!< Called from terminal layer when raw CMUX
                                    protocol data available */

  struct CMuxFrame; /*!< Forward declare the Frame struct, used in protocol
                       decoders */
  /**
  ...
  std::function<bool(uint8_t *data, size_t len)>
      read_cb[MAX_TERMINALS_NUM]; /*!< Function pointers to read callbacks */
...
/**
 * @brief This represents a specific instance of a CMUX virtual terminal. This
 * class also implements Terminal interface and as such could be used as a DTE's
 * terminal.
 */
class CMuxInstance : public Terminal {
public:
  explicit CMuxInstance(std::shared_ptr<CMux> parent, int i)
      : cmux(std::move(parent)), instance(i) {}
