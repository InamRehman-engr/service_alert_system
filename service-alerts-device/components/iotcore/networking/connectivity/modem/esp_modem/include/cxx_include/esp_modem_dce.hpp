...
/**
 * @brief General DCE class templated on a specific module. It is responsible
 * for all the necessary transactions related to switching modes and consequent
 * synergy with aggregated objects of DTE, Netif and a specific Module
 */
template <class SpecificModule> class DCE_T {
  static_assert(std::is_base_of<ModuleIf, SpecificModule>::value,
                "DCE must be instantiated with Module class only");

public:
  explicit DCE_T(const std::shared_ptr<DTE> &dte,
                 std::shared_ptr<SpecificModule> dev, esp_netif_t *netif)
  ...
  bool recover() { return dte->recover(); }
...
/**
 * @brief Common abstraction of the modem DCE, specialized by the GenericModule
 * which is a parent class for the supported devices and most common modems, as
 * well.
 */
...
#define ESP_MODEM_DECLARE_DCE_COMMAND(name, return_type, num, ...)             \
  template <typename... Agrs> return_type name(Agrs &&...args) {               \
