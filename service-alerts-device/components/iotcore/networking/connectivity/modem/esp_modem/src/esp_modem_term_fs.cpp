...
struct File {
  explicit File(const esp_modem_dte_config *config)
      : fd(config->vfs_config.fd), deleter(config->vfs_config.deleter),
        resource(config->vfs_config.resource) {}
...
public:
  ...
  void stop() override { signal.clear(TASK_START); }
...
create_vfs_terminal(const esp_modem_dte_config *config) {
  TRY_CATCH_RET_NULL(auto term = std::make_unique<FdTerminal>(config);
