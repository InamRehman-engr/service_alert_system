 ...
 * @note: Remote command:
 * socat TCP-L:2222
 * GOPEN:/dev/ttyS0,ispeed=115200,ospeed=1152000,b115200,raw,echo=0
 */
static esp_err_t hostname_to_fd(const char *host, int port, int *fd) {
  ...
  *fd = socket(address_info->ai_family, address_info->ai_socktype,
  ...
  if (*fd < 0) {
    ESP_LOGE(TAG, "Failed to create socket (family %d socktype %d protocol %d)",
             address_info->ai_family, address_info->ai_socktype,
             address_info->ai_protocol);
  ...
  if (address_info->ai_family == AF_INET) {
    ...
    ESP_LOGI(TAG, "[sock=%d] Resolved IPv4 address: %s", *fd,
...
static void vfs_destroy_socket(int fd,
...
bool vfs_create_socket(struct esp_modem_vfs_socket_creator *config,
                       struct esp_modem_vfs_term_config *created_config) {
  if (config == nullptr || created_config == nullptr) {
    return false;
  }
  TRY_CATCH_OR_DO(int fd = -1; ESP_MODEM_THROW_IF_ERROR(
                      hostname_to_fd(config->host_name, config->port, &fd));
                  ...
                  created_config->deleter = vfs_destroy_socket;, return false)
  return true;
}
