...
//
// Repeat all declarations and forward to the AT commands defined in
// esp_modem::dce_commands:: namespace
//
#define ESP_MODEM_DECLARE_DCE_COMMAND(name, return_type, arg_nr, ...)          \
  return_type GenericModule::name(__VA_ARGS__) {                               \
    return esp_modem::dce_commands::name(dte.get() ARGS(arg_nr));              \
  }
...
command_result SIM7600::set_network_bands(const std::string &mode,
...
command_result SIM7600::get_gnss_power_mode(int &mode) {
  return dce_commands::get_gnss_power_mode_sim76xx(dte.get(), mode);
}

command_result A76XX::set_gnss_power_mode(int mode) {
  return dce_commands::set_gnss_power_mode_A76xx(dte.get(), mode);
}

command_result A76XX::get_gnss_power_mode(int &mode) {
  return dce_commands::get_gnss_power_mode_A76xx(dte.get(), mode);
}

command_result SIM800::get_gps_location(esp_modem::gps &gps) {
  return dce_commands::get_gps_location_sim868(dte.get(), gps);
}

command_result SIM7600::get_gps_location(esp_modem::gps &gps) {
  return dce_commands::get_gps_location_sim76xx(dte.get(), gps);
}

command_result A76XX::get_gps_location(esp_modem::gps &gps) {
  return dce_commands::get_gps_location_A76XX(dte.get(), gps);
}
