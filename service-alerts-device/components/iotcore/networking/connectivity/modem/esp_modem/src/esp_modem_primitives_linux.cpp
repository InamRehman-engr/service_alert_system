...
SignalGroup::SignalGroup()
    : event_group(std::make_unique<SignalGroupInternal>()) {}
...
bool SignalGroup::wait(uint32_t flags, uint32_t time_ms) {
  std::unique_lock<std::mutex> lock(event_group->m);
  return event_group->notify.wait_for(
      lock, std::chrono::milliseconds(time_ms), [&] {
...
Task::~Task() { task_handle.join(); }

void Task::Delete() {}

void Task::Relinquish() { usleep(0); }

void Task::Delay(uint32_t ms) { usleep(ms * 1000); }
