...
esp_err_t Netif::esp_modem_post_attach(esp_netif_t *esp_netif, void *args) {
  ...
  // check if PPP error events are enabled, if not, do enable the error
  // occurred/state changed to notify the modem layer when switching modes
  esp_netif_ppp_config_t ppp_config = {};
  ppp_config.ppp_phase_event_enabled =
      true; // assuming phase enabled, as earlier IDFs
  ppp_config.ppp_error_event_enabled =
      false; // don't provide cfg getters so we enable both events
...
Netif::Netif(std::shared_ptr<DTE> e, esp_netif_t *ppp_netif)
  ...
  ESP_MODEM_THROW_IF_ERROR(esp_event_handler_register(
      NETIF_PPP_STATUS, ESP_EVENT_ANY_ID, &on_ppp_changed, (void *)this));
  ESP_MODEM_THROW_IF_ERROR(esp_event_handler_register(
      IP_EVENT, IP_EVENT_PPP_GOT_IP, esp_netif_action_connected, ppp_netif));
  ESP_MODEM_THROW_IF_ERROR(
      esp_event_handler_register(IP_EVENT, IP_EVENT_PPP_LOST_IP,
                                 esp_netif_action_disconnected, ppp_netif));
...
void Netif::wait_until_ppp_exits() { signal.wait(PPP_EXIT, 30000); }
