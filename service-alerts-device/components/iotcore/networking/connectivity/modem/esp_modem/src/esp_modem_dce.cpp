...
static bool exit_data(DTE &dte, ModuleIf &device, Netif &netif) {
  ...
  dte.set_read_cb([&netif, weak_signal](uint8_t *data, size_t len) -> bool {
    // post the transitioning data to the network layers if it contains PPP SOF
    // marker
    if (memchr(data, 0x7E, len)) {
      ESP_LOG_BUFFER_HEXDUMP("esp-modem: debug_data (PPP)", data, len,
                             ESP_LOG_DEBUG);
      netif.receive(data, len);
    }
    // treat the transitioning data as a textual message if it contains a
    // newline char
    if (memchr(data, '\n', len)) {
      ESP_LOG_BUFFER_HEXDUMP("esp-modem: debug_data (CMD)", data, len,
                             ESP_LOG_DEBUG);
      const auto pass =
          std::list<std::string_view>({"NO CARRIER", "DISCONNECTED"});
 ...
 * UNDEF <----> any
 */
bool DCE_Mode::set_unsafe(DTE *dte, ModuleIf *device, Netif &netif,
                          modem_mode m) {
  switch (m) {
...
modem_mode DCE_Mode::get() { return mode; }
