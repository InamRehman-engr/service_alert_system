...
void CMux::send_disconnect(size_t i) {
  if (i == 0) { // control terminal
    uint8_t frame[] = {SOF_MARKER, 0x3, 0xEF, 0x5, 0xC3, 0x1, 0xF2, SOF_MARKER};
    term->write(frame, 8);
  } else { // separate virtual terminal
    uint8_t frame[] = {SOF_MARKER, 0x3, FT_DISC | PF, 0x1, 0, SOF_MARKER};
    frame[1] |= i << 2;
...
struct CMux::CMuxFrame {
  uint8_t
      *ptr;   /*!< pointer to the currently processing byte of the CMUX frame */
...
bool CMux::data_available(uint8_t *data, size_t len) {
  if (data && (type & FT_UIH) == FT_UIH && len > 0 &&
      dlci > 0) { // valid payload on a virtual term
  ...
  } else if (data == nullptr && type == (FT_UA | PF) &&
             len == 0) { // notify the initial SABM command
    Scoped<Lock> l(lock);
    sabm_ack = dlci;
  ...
  } else if ((type & FT_UIH) == FT_UIH &&
             dlci == 0) { // notify the internal DISC command
...
bool CMux::on_recovery(CMuxFrame &frame) {
  ...
  recover_ptr =
      static_cast<uint8_t *>(memchr(frame.ptr, SOF_MARKER, frame.len));
...
bool CMux::on_header(CMuxFrame &frame) {
  if (frame.len > 0 && frame_header_offset == 1 && frame.ptr[0] == SOF_MARKER) {
    // Previously trailing SOF interpreted as heading SOF, remove it and restart
    // HEADER
    frame.advance();
    return true;
  }
  ...
  if ((frame_header[3] & 1) == 0) {
    ...
    frame_header_offset +=
        payload_offset -
        1; // rewind frame_header back to hold only 6 bytes size
  } else
...
bool CMux::on_payload(CMuxFrame &frame) {
  ESP_LOGD("CMUX", "Payload frame: dlci:%02x type:%02x payload:%d available:%d",
           dlci, type, payload_len, frame.len);
...
#ifdef DEFRAGMENT_CMUX_PAYLOAD
    auto data_to_read =
        buffer.size - 128; // keep 128 (max CMUX payload) backup buffer)
    if (payload_start) {
      ...
      if (data + data_to_read >= data_end) {
        ESP_LOGW("CUMX", "Failed to defragment longer payload (payload=%d)",
                 payload_len);
        // If you experience this error, your device uses longer payloads while
        // the configured buffer is too small to defragment the payload
        // properly. To resolve this issue you can:
        // * Either increase `dte_buffer_size`
        // * Or disable `ESP_MODEM_CMUX_DEFRAGMENT_PAYLOAD` in menuconfig

        // Attempts to process the data accumulated so far (rely on upper layers
...
bool CMux::init() {
  ...
  for (size_t i = 0; i < 3; i++) {
    ...
    if (i > 1) { // wait for each virtual terminal to settle MSC (no need for
                 // control term, DLCI=0)
...
void esp_modem::CMux::recover_protocol(protocol_mismatch_reason reason) {
  ESP_LOGW("CMUX", "Restarting CMUX state machine (reason: %d)",
