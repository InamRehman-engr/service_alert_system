...
DTE::DTE(const esp_modem_dte_config *config, std::unique_ptr<Terminal> terminal)
    : buffer(config->dte_buffer_size), cmux_term(nullptr),
      primary_term(std::move(terminal)), secondary_term(primary_term),
      mode(modem_mode::UNDEF) {
  set_command_callbacks();
}

DTE::DTE(std::unique_ptr<Terminal> terminal)
    : buffer(dte_default_buffer_size), cmux_term(nullptr),
      primary_term(std::move(terminal)), secondary_term(primary_term),
      mode(modem_mode::UNDEF) {
  set_command_callbacks();
}

DTE::DTE(const esp_modem_dte_config *config, std::unique_ptr<Terminal> t,
         std::unique_ptr<Terminal> s)
    : buffer(config->dte_buffer_size), cmux_term(nullptr),
      primary_term(std::move(t)), secondary_term(std::move(s)),
...
void DTE::set_command_callbacks() {
  primary_term->set_read_cb([this](uint8_t *data, size_t len) {
    ...
    if (data) {
      ...
      // cannot inflate and the processing hasn't finishes in the first
      // iteration, but continue (will post next fragments to the parser, since
      // we might be just missing a last token or OK
      return false;
#endif
    }
    // data == nullptr: Terminals which request users to read current data
    // we're able to use DTE's buffer to defragment it; as long as we consume
    // less that the buffer size
    if (buffer.size > buffer.consumed) {
      data = buffer.get();
      len = primary_term->read(data + buffer.consumed,
    ...
    // we have used the entire DTE's buffer, need to use the inflatable buffer
    // to continue
#ifdef CONFIG_ESP_MODEM_USE_INFLATABLE_BUFFER_IF_NEEDED
    if (inflatable.consumed == 0) {
...
command_result DTE::command(const std::string &command, got_line_cb got_line,
                            uint32_t time_ms, const char separator) {
...
command_result DTE::command(const std::string &cmd, got_line_cb got_line,
                            uint32_t time_ms) {
...
bool DTE::set_mode(modem_mode m) {
  ...
  if (m == modem_mode::DATA_MODE) {
    if (mode == modem_mode::CMUX_MODE || mode == modem_mode::CMUX_MANUAL_MODE ||
        mode == modem_mode::DUAL_MODE) {
      // mode stays the same, but need to swap terminals (as command has been
      // switched)
  ...
  if (m == modem_mode::CMUX_MANUAL_EXIT &&
      (mode == modem_mode::CMUX_MANUAL_MODE || mode == modem_mode::UNDEF)) {
    if (exit_cmux()) {
  ...
  if (m == modem_mode::CMUX_MANUAL_SWAP &&
      (mode == modem_mode::CMUX_MANUAL_MODE || mode == modem_mode::UNDEF)) {
    secondary_term.swap(primary_term);
    set_command_callbacks();
    return true;
...
void DTE::set_read_cb(std::function<bool(uint8_t *, size_t)> f) {
  ...
  secondary_term->set_read_cb([this](uint8_t *data, size_t len) {
    if (!data) { // if no data available from terminal callback -> need to
                 // explicitly read some
      data = buffer.get();
...
bool DTE::command_cb::process_line(uint8_t *data, size_t consumed, size_t len) {
  if (result != command_result::TIMEOUT) {
    return false; // this line has been processed already (got OK or FAIL
                  // previously)
  }
