...
static command_result generic_get_string(CommandableIf *t,
  ...
  return t->command(
      ...
      [&](uint8_t *data, size_t len) {
        ...
        while ((pos = response.find('\n')) != std::string::npos) {
          ...
          for (auto it = token.end() - 1; it > token.begin(); it--)
            if (*it == '\r' || *it == '\n') {
...
static command_result generic_get_string(CommandableIf *t,
  ...
  return t->command(
      ...
      [&](uint8_t *data, size_t len) {
        ...
        while ((pos = response.find('\n')) != std::string::npos) {
          std::string_view token = response.substr(0, pos);
          for (auto it = token.end() - 1; it > token.begin();
               it--) // strip trailing CR or LF
            if (*it == '\r' || *it == '\n') {
              token.remove_suffix(1);
            }
          ESP_LOGV(TAG, "Token: {%.*s}\n", static_cast<int>(token.size()),
...
command_result generic_get_string(CommandableIf *t, const std::string &command,
                                  std::string &output, uint32_t timeout_ms) {
  ESP_LOGV(TAG, "%s", __func__);
  std::string_view out;
...
command_result generic_command_common(CommandableIf *t,
                                      const std::string &command,
                                      uint32_t timeout_ms) {
  ESP_LOGV(TAG, "%s", __func__);
...
command_result get_battery_status(CommandableIf *t, int &voltage, int &bcs,
                                  int &bcl) {
  ESP_LOGV(TAG, "%s", __func__);
  std::string_view out;
  ...
  while ((pos = out.find(',')) != std::string::npos) {
    if (std::from_chars(out.data(), out.data() + pos, value).ec ==
  ...
  if (std::from_chars(out.data(), out.data() + out.size(), voltage).ec ==
      std::errc::invalid_argument) {
    return command_result::FAIL;
  }
  return command_result::OK;
...
command_result get_battery_status_sim7xxx(CommandableIf *t, int &voltage,
                                          int &bcs, int &bcl) {
  ESP_LOGV(TAG, "%s", __func__);
  std::string_view out;
  auto ret = generic_get_string(t, "AT+CBC\r", out);
  ...
  int volt, fraction;
  if (std::from_chars(out.data() + num_pos, out.data() + dot_pos, volt).ec ==
      std::errc::invalid_argument) {
    return command_result::FAIL;
  }
  if (std::from_chars(out.data() + dot_pos + 1, out.data() + out.size() - 1,
                      fraction)
...
command_result set_flow_control(CommandableIf *t, int dce_flow, int dte_flow) {
  ESP_LOGV(TAG, "%s", __func__);
  return generic_command_common(t, "AT+IFC=" + std::to_string(dce_flow) + "," +
                                       std::to_string(dte_flow) + "\r");
}
...
command_result get_operator_name(CommandableIf *t, std::string &operator_name,
  ...
  while (pos != std::string::npos) {
    ...
    if (property++ == 2) { // operator name is after second comma (as a 3rd
                           // property of COPS string)
      operator_name = out.substr(++pos);
      auto additional_comma =
          operator_name.find(','); // check for the optional ACT
      if (additional_comma != std::string::npos &&
          std::from_chars(operator_name.data() + additional_comma + 1,
                          operator_name.data() + operator_name.length(), act)
                  .ec != std::errc::invalid_argument) {
...
command_result send_sms(CommandableIf *t, const std::string &number,
  ...
  auto ret = t->command(
      ...
      [&](uint8_t *data, size_t len) {
        std::string_view response((char *)data, len);
        ESP_LOGD(TAG, "Send SMS response %.*s",
...
command_result read_pin(CommandableIf *t, bool &pin_ok) {
  ...
  if (out.find("SIM PIN") != std::string::npos ||
      out.find("SIM PUK") != std::string::npos) {
    pin_ok = false;
    return command_result::OK;
  }
  ...
  return command_result::FAIL; // Neither pin-ok, nor waiting for pin/puk ->
                               // mark as error
}
...
command_result get_signal_quality(CommandableIf *t, int &rssi, int &ber) {
  ...
  if (std::from_chars(out.data() + rssi_pos, out.data() + ber_pos, rssi).ec ==
      std::errc::invalid_argument) {
    return command_result::FAIL;
  }
  if (std::from_chars(out.data() + ber_pos + 1, out.data() + out.size(), ber)
          .ec == std::errc::invalid_argument) {
    return command_result::FAIL;
  }
  return command_result::OK;
...
command_result set_operator(CommandableIf *t, int mode, int format,
                            const std::string &oper) {
  ESP_LOGV(TAG, "%s", __func__);
  return generic_command_common(t,
                                "AT+COPS=" + std::to_string(mode) + "," +
                                    std::to_string(format) + ",\"" + oper +
                                    "\"\r",
                                90000);
}
...
command_result get_network_attachment_state(CommandableIf *t, int &state) {
  ...
  if (std::from_chars(out.data() + pos, out.data() + out.size(), state).ec ==
      std::errc::invalid_argument) {
    return command_result::FAIL;
  }
...
command_result set_network_bands(CommandableIf *t, const std::string &mode,
  ...
  return generic_command_common(t, "AT+CBANDCFG=\"" + mode + "\"," +
                                       band_string + "\r");
}

// mode is expected to be 64bit string (in hex)
// any_mode = "0xFFFFFFFF7FFFFFFF";
command_result set_network_bands_sim76xx(CommandableIf *t,
  ...
  for (int i = 0; i < size; ++i) {
    ...
    auto band = bands[i] - 1; // Sim7600 has 0-indexed band selection (band 20
                              // has to be shifted 19 places)
    band_bits |= 1 << band;
  }
...
command_result get_network_system_mode(CommandableIf *t, int &mode) {
  ...
  if (std::from_chars(out.data() + mode_pos, out.data() + out.size(), mode)
          .ec == std::errc::invalid_argument) {
    return command_result::FAIL;
  }
...
command_result get_gnss_power_mode(CommandableIf *t, int &mode) {
  ...
  if (std::from_chars(out.data() + pos, out.data() + out.size(), mode).ec ==
      std::errc::invalid_argument) {
    return command_result::FAIL;
  }
...
command_result get_gnss_power_mode_sim76xx(CommandableIf *t, int &mode) {
  ESP_LOGV(TAG, "%s", __func__);
  std::string_view out;
  auto ret = generic_get_string(t, "AT+CGPS?\r", out);
  if (ret != command_result::OK) {
    return ret;
  }
  constexpr std::string_view pattern = "+CGPS: ";
  constexpr int pos = pattern.size();
  if (out.find(pattern) == std::string::npos) {
    return command_result::FAIL;
  }
...
command_result get_gnss_power_mode_A76xx(CommandableIf *t, int &mode) {
  ESP_LOGV(TAG, "%s", __func__);
  std::string_view out;
  auto ret = generic_get_string(t, "AT+CGNSSPWR?\r", out);
  if (ret != command_result::OK) {
    return ret;
  }
  constexpr std::string_view pattern = "+CGNSSPWR: ";
  constexpr int pos = pattern.size();
  if (out.find(pattern) == std::string::npos) {
    return command_result::FAIL;
  }

  if (std::from_chars(out.data() + pos, out.data() + pos + 1, mode).ec ==
      std::errc::invalid_argument) {
    return command_result::FAIL;
  }
...
command_result set_gnss_power_mode_A76xx(CommandableIf *t, int mode) {
  ESP_LOGV(TAG, "%s", __func__);
  return generic_command_common(t,
                                "AT+CGNSSPWR=" + std::to_string(mode) + "\r");
}

command_result get_gps_location(CommandableIf *t, gps &gps) {
  ESP_LOGV(TAG, "%s", __func__);
  std::string_view out;
  auto ret = generic_get_string(t, "AT+CGNSINF\r", out);
  // Parse GPS Data to gps
  std::cout << "SIM7070G GPS Response: " << out << std::endl;
  ret = get_gnss_information_sim70xx_lib(out, gps);
  return ret;
}
command_result get_gps_location_sim868(CommandableIf *t, gps &gps) {
  ESP_LOGV(TAG, "%s", __func__);
  char out[200] = {0};
  auto ret = generic_get_string(t, "AT+CGNSINF\r", out);
  std::cout << "SIM868 GPS Response: " << out << std::endl;
  // Parse GPS Data to gps
  ret = get_gnss_information_sim868_lib(out, gps);
  return ret;
}

command_result get_gps_location_sim76xx(CommandableIf *t, gps &gps) {
  ESP_LOGV(TAG, "%s", __func__);
  char out[200] = {0};
  auto ret = generic_get_string(t, "AT+CGNSSINFO\r", out);
  std::cout << "SIM7600G GPS Response: " << out << std::endl;
  // Parse GPS Data to gps
  ret = get_gnss_information_sim76xx_lib(out, gps);
  return ret;
}

command_result get_gps_location_A76XX(CommandableIf *t, gps &gps) {
  ESP_LOGV(TAG, "%s", __func__);
  char out[200] = {0};
  auto ret = generic_get_string(t, "AT+CGPSINFO\r", out);
  std::cout << "A76XX GPS Response: " << out << std::endl;
  // Parse GPS Data to gps
  ret = get_gnss_information_A76XX_lib(out, gps);
  return ret;
}
